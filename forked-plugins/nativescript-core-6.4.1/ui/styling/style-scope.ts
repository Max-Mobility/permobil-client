...
import { Keyframes as KeyframesDefinition, Node as CssNode, parse as parseCss, SyntaxTree } from '../../css';
...
import { isCssCalcExpression, isCssVariable, isCssVariableExpression, unsetValue, _evaluateCssCalcExpression, _evaluateCssVariableExpression } from '../core/properties';
import { View } from '../core/view';
import { ViewBase } from '../core/view-base';
import * as capm from './css-animation-parser';
import { ChangeMap, fromAstNodes, Node, RuleSet, SelectorCore, SelectorsMap, SelectorsMatch } from './css-selector';
 ...
 */
function evaluateCssExpressions(
  view: ViewBase,
  property: string,
  value: string
) {
...
class CSSSource {
  ...
  @profile
  public static resolveCSSPathFromURL(
    url: string,
    importSource?: string
  ): string {
  ...
  private createSelectorsFromImports(): RuleSet[] {
    const imports = this._ast['stylesheet']['rules'].filter(
      r => r.type === 'import'
    );
...
export function addTaggedAdditionalCSS(
  cssText: string,
  tag?: string | Number
): Boolean {
...
const onCssChanged = profile(
  ...
  (args: application.CssChangedEventData) => {
    if (args.cssText) {
...
export const loadAppCSS = profile(
  ...
  (args: application.LoadAppCSSEventData) => {
    loadCss(args.cssFile);
...
export class CssState {
  static emptyChangeMap: Readonly<ChangeMap<ViewBase>> = Object.freeze(
    new Map()
  );
  static emptyPropertyBag: Readonly<{}> = Object.freeze({});
  static emptyAnimationArray: ReadonlyArray<
    kam.KeyframeAnimation
  > = Object.freeze([]);
  ...
  public isSelectorsLatestVersionApplied(): boolean {
    ...
    if (!view) {
      traceWrite(
        ...
        traceCategories.Style,
        traceMessageType.warn
      );
  ...
  private playKeyframeAnimations(matchingSelectors: SelectorCore[]): void {
    ...
    matchingSelectors.forEach(selector => {
      let ruleAnimations: kam.KeyframeAnimationInfo[] =
        selector.ruleset[animationsSymbol];
      if (ruleAnimations) {
        ensureKeyframeAnimationModule();
        for (let animationInfo of ruleAnimations) {
          let animation = keyframeAnimationModule.KeyframeAnimation.keyframeAnimationFromInfo(
            animationInfo
          );
          if (animation) {
  ...
  private setPropertyValues(matchingSelectors: SelectorCore[]): void {
    ...
    for (const property in newPropertyValues) {
      ...
      isCssExpressionInUse =
        isCssExpressionInUse ||
        isCssVariableExpression(value) ||
        isCssCalcExpression(value);
    }

    if (isCssExpressionInUse) {
      // Evalute css-expressions to get the latest values.
      for (const property in newPropertyValues) {
        const value = evaluateCssExpressions(
          view,
          property,
...
export class StyleScope {
  ...
  private setCss(cssString: string, cssFileName?): void {
    ...
    const cssFile = CSSSource.fromSource(
      cssString,
      this._keyframes,
      cssFileName
    );
  ...
  private appendCss(cssString: string, cssFileName?): void {
    ...
    let parsedCssSelectors = cssString
  ...
  public getKeyframeAnimationWithName(
    animationName: string
  ): kam.KeyframeAnimationInfo {
  ...
  private _createSelectors() {
    ...
    if (toMerge.length > 0) {
      this._mergedCssSelectors = toMerge
        .filter(m => !!m)
        .reduce((merged, next) => merged.concat(next), []);
...
function resolveFilePathFromImport(
  importSource: string,
  fileName: string
): string {
