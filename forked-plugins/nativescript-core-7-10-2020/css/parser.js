...
function parseHexColor(text, start) {
  ...
  if (hex.length === 8) {
    ...
    argb = parseInt(
      '0xFF' + hex[0] + hex[0] + hex[1] + hex[1] + hex[2] + hex[2]
    );
  }
...
(function (colors) {
  ...
  colors[(colors['lightgoldenrodyellow'] = 4294638290)] =
...
var backgroundRepeatKeywords = new Set([
  'repeat',
  'repeat-x',
  'repeat-y',
  'no-repeat'
...
var backgroundPositionKeywords = Object.freeze(
  new Set(['left', 'right', 'top', 'bottom', 'center'])
);
...
function parseBackgroundPosition(text, start, keyword) {
  ...
  if (keyword && backgroundPositionKeywords.has(keyword.value)) {
    ...
    var firstLength =
      firstDirection !== 'center' && parsePercentageOrLength(text, end);
    if (firstLength) {
      end = firstLength.end;
    }
    ...
    if (secondKeyword && backgroundPositionKeywords.has(secondKeyword.value)) {
      ...
      var secondDirection =
        backgroundPositionKeywordsDirection[secondKeyword.end];
      if (firstDirection === secondDirection && firstDirection !== 'center') {
        return null;
      }
      var secondLength =
        secondDirection !== 'center' && parsePercentageOrLength(text, end);
      if (secondLength) {
        end = secondLength.end;
      }
      if (
...
function parseSimpleIdentifierSelector(text, start) {
  ...
  var result = simpleIdentifierSelectorRegEx.exec(
    text.replace(unicodeEscapeRegEx, function (_, c) {
      return '\\' + String.fromCodePoint(parseInt(c.trim(), 16));
    })
  );
  if (!result) {
    return null;
...
var CSS3Parser = (function () {
  ...
  CSS3Parser.prototype.consumeAToken = function () {
    ...
    switch (char) {
      ...
      case '-':
        return (
          this.consumeANumericToken() ||
          this.consumeAnIdentLikeToken() ||
          this.consumeCDC() ||
  ...
  CSS3Parser.prototype.consumeAURLToken = function () {
    ...
    if (nextInputCodePoint === '"' || nextInputCodePoint === "'") {
      ...
      this.consumeAWhitespace();
      if (
        this.text[this.nextInputCodePointIndex] === ')' ||
        this.nextInputCodePointIndex >= this.text.length
      ) {
        this.nextInputCodePointIndex++;
