...
(function (global) {
  ...
  function MemberExpression(object, property, accessor) {
    ...
    this.property =
      !this.computed || this.simplePath ? property : getFn(property);
  }
      ...
      if (!this.fullPath_) {
        var parts =
          this.object instanceof MemberExpression
            ...
            : [this.object.name];
        parts.push(
          this.property instanceof IdentPath
            ? this.property.name
            : this.property.value
        );
  ...
  ASTDelegate.prototype = {
    ...
    createObjectExpression: function (properties) {
      ...
      return function (model, observer, filterRegistry) {
        var obj = {};
        for (var i = 0; i < properties.length; i++)
          obj[properties[i].key] = properties[i].value(
            model,
            observer,
            filterRegistry
          );
  ...
  Expression.prototype = {
    getValue: function (model, isBackConvert, changedModel, observer) {
      ...
      for (var i = 0; i < this.filters.length; i++) {
        value = this.filters[i].transform(
          model.context,
          observer,
          model.context,
          isBackConvert,
    ...
    setValue: function (model, newValue, filterRegistry) {
      ...
      while (count-- > 0) {
        newValue = this.filters[count].transform(
          model,
          undefined,
          filterRegistry,
          true,
