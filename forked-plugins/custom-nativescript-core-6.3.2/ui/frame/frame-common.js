...
var frame_interfaces_1 = require('./frame-interfaces');
...
__export(require('./frame-interfaces'));
...
var FrameBase = (function(_super) {
  ...
  FrameBase.getFrameById = function(id) {
    return frame_stack_1.frameStack.find(function(frame) {
      return frame.id && frame.id === id;
    });
  };
  FrameBase.topmost = function() {
    return frame_stack_1.topmost();
  };
  FrameBase.goBack = function() {
    var top = FrameBase_1.topmost();
  ...
  FrameBase.reloadPage = function() {};
  FrameBase._stack = function() {
    return frame_stack_1.frameStack;
  };
  FrameBase.prototype._addChildFromBuilder = function(name, value) {
    throw new Error(
      "Frame should not have a view. Use 'defaultPage' property instead."
    );
  };
  ...
  FrameBase.prototype.canGoBack = function() {
    ...
    this._navigationQueue.forEach(function(item) {
      ...
      var isBackNavigation =
        item.navigationType === frame_interfaces_1.NavigationType.back;
      if (isBackNavigation) {
  ...
  FrameBase.prototype.goBack = function(backstackEntry) {
    ...
    var navigationContext = {
      ...
      navigationType: frame_interfaces_1.NavigationType.back
    };
  ...
  FrameBase.prototype.navigate = function(param) {
    ...
    var page = builder_1.Builder.createViewFromEntry(entry);
    var backstackEntry = {
      entry: entry,
      resolvedPage: page,
      navDepth: undefined,
    ...
    var navigationContext = {
      ...
      navigationType: frame_interfaces_1.NavigationType.forward
    };
    this._navigationQueue.push(navigationContext);
    this._processNextNavigationEntry();
  };
  ...
  FrameBase.prototype.setCurrent = function(entry, navigationType) {
    ...
    var isBack = navigationType === frame_interfaces_1.NavigationType.back;
    if (isBack) {
      this._pushInFrameStack();
    }
  ...
  FrameBase.prototype._updateBackstack = function(entry, navigationType) {
    ...
    var isBack = navigationType === frame_interfaces_1.NavigationType.back;
    var isReplace =
      navigationType === frame_interfaces_1.NavigationType.replace;
  ...
  FrameBase._isEntryBackstackVisible = function(entry) {
    ...
    var backstackHidden =
      backstackVisibleValue !== undefined && !backstackVisibleValue;
    return !backstackHidden;
  };
  FrameBase.prototype._updateActionBar = function(
    page,
  ...
  FrameBase.prototype._processNextNavigationEntry = function() {
    ...
    if (this._navigationQueue.length > 0) {
      ...
      var isBackNavigation =
        ...
        frame_interfaces_1.NavigationType.back;
      if (isBackNavigation) {
  ...
  FrameBase.prototype._goBackCore = function(backstackEntry) {
    if (view_1.traceEnabled()) {
      view_1.traceWrite(
        'GO BACK CORE(' +
          this._backstackEntryTrace(backstackEntry) +
          '); currentPage: ' +
          this.currentPage,
        view_1.traceCategories.Navigation
      );
    }
  };
  ...
  FrameBase.prototype._onNavigatingTo = function(backstackEntry, isBack) {
    ...
    backstackEntry.resolvedPage.onNavigatingTo(
      backstackEntry.entry.context,
      isBack,
      backstackEntry.entry.bindingContext
    );
  };
  ...
  FrameBase.prototype._pushInFrameStackRecursive = function() {
    ...
    var framesToPush = [];
    for (
      var _i = 0, frameStack_1 = frame_stack_1.frameStack;
      _i < frameStack_1.length;
      _i++
    ) {
    ...
    for (
      var _a = 0, framesToPush_1 = framesToPush;
      _a < framesToPush_1.length;
      _a++
    ) {
  ...
  FrameBase.prototype._handleLivesync = function(context) {
    ...
    if (
      this.currentPage &&
      view_common_1.viewMatchesModuleContext(this.currentPage, context, [
        'markup',
        'script'
      ])
    ) {
      view_1.traceWrite(
        'Change Handled: Replacing page ' + context.path,
        view_1.traceCategories.Livesync
      );
  ...
  FrameBase.prototype.replacePage = function(pagePath) {
    ...
    var contextModuleName = module_name_sanitizer_1.sanitizeModuleName(
      pagePath
    );
    var newPage = builder_1.Builder.createViewFromEntry({
...
function getFrameById(id) {
  console.log(
    'getFrameById() is deprecated. Use Frame.getFrameById() instead.'
  );
