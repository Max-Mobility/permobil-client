...
var view_helper_1 = require('../core/view/view-helper');
...
var Frame = (function(_super) {
  ...
  Frame.prototype._navigateCore = function(backstackEntry) {
    ...
    if (isReplace) {
      ...
      navigationTransition = this._getNavigationTransition(
    ...
    if (!Frame._isEntryBackstackVisible(this._currentEntry) || isReplace) {
      ...
      if (trace_1.isEnabled()) {
        trace_1.write(
          this +
            '.setViewControllersAnimated([originalControllers - lastController + ' +
            viewController +
            '], ' +
            animated +
            '); depth = ' +
  ...
  Frame.prototype._goBackCore = function(backstackEntry) {
    ...
    var animated = this._currentEntry
...
var TransitionDelegate = (function(_super) {
  ...
  TransitionDelegate.prototype.animationWillStart = function(
    animationID,
    context
  ) {
    if (trace_1.isEnabled()) {
      trace_1.write('START ' + this._id, trace_1.categories.Transition);
    }
  };
  TransitionDelegate.prototype.animationDidStop = function(
    animationID,
    finished,
    context
  ) {
    if (finished) {
      if (trace_1.isEnabled()) {
        trace_1.write('END ' + this._id, trace_1.categories.Transition);
      }
    } else {
      if (trace_1.isEnabled()) {
        trace_1.write('CANCEL ' + this._id, trace_1.categories.Transition);
      }
    }
...
var UINavigationControllerAnimatedDelegate = (function(_super) {
  ...
  UINavigationControllerAnimatedDelegate.prototype.navigationControllerAnimationControllerForOperationFromViewControllerToViewController = function(
    ...
    if (trace_1.isEnabled()) {
      trace_1.write(
        'UINavigationControllerImpl.navigationControllerAnimationControllerForOperationFromViewControllerToViewController(' +
          operation +
          ', ' +
          fromVC +
          ', ' +
          toVC +
          '), transition: ' +
          JSON.stringify(navigationTransition),
        trace_1.categories.NativeLifecycle
      );
    }
    var curve = _getNativeCurve(navigationTransition);
    var animationController = fragment_transitions_1._createIOSAnimatedTransitioning(
  ...
  UINavigationControllerAnimatedDelegate.ObjCProtocols = [
...
var UINavigationControllerImpl = (function(_super) {
  ...
  UINavigationControllerImpl.prototype.animateWithDuration = function(
    navigationTransition,
    nativeTransition,
    transitionType,
    baseCallback
    ...
    var _this = this;
    var duration = navigationTransition.duration
      ? navigationTransition.duration / 1000
      : _defaultTransitionDuration;
    var curve = _getNativeCurve(navigationTransition);
    var transitionTraced = trace_1.isCategorySet(trace_1.categories.Transition);
  ...
  UINavigationControllerImpl.prototype.pushViewControllerAnimated = function(
    viewController,
    animated
  ) {
    var _this = this;
    ...
    if (trace_1.isEnabled()) {
      trace_1.write(
        'UINavigationControllerImpl.pushViewControllerAnimated(' +
          viewController +
          ', ' +
          animated +
          '); transition: ' +
    ...
    if (!animated || !navigationTransition || !nativeTransition) {
      _super.prototype.pushViewControllerAnimated.call(
        this,
        viewController,
        animated
      );
  ...
  UINavigationControllerImpl.prototype.setViewControllersAnimated = function(
    viewControllers,
    animated
  ) {
    var _this = this;
    ...
    if (trace_1.isEnabled()) {
      trace_1.write(
        'UINavigationControllerImpl.setViewControllersAnimated(' +
          viewControllers +
          ', ' +
          animated +
          '); transition: ' +
    ...
    var nativeTransition = _getNativeTransition(navigationTransition, true);
    if (!animated || !navigationTransition || !nativeTransition) {
      _super.prototype.setViewControllersAnimated.call(
        this,
        viewControllers,
        animated
      );
  ...
  UINavigationControllerImpl.prototype.popViewControllerAnimated = function(
    animated
  ) {
    var _this = this;
    ...
    if (trace_1.isEnabled()) {
      trace_1.write(
        'UINavigationControllerImpl.popViewControllerAnimated(' +
          animated +
          '); transition: ' +
          JSON.stringify(navigationTransition),
        trace_1.categories.NativeLifecycle
    ...
    if (
      navigationTransition &&
      navigationTransition.name === NON_ANIMATED_TRANSITION
    ) {
  ...
  UINavigationControllerImpl.prototype.popToViewControllerAnimated = function(
    viewController,
    animated
  ) {
    var _this = this;
    ...
    if (
      ...
      return _super.prototype.popToViewControllerAnimated.call(
        this,
        viewController,
        false
      );
    ...
    this.animateWithDuration(
      ...
      'popTo',
      function() {
        _super.prototype.popToViewControllerAnimated.call(
          _this,
          viewController,
          false
        );
  ...
  UINavigationControllerImpl.prototype.traitCollectionDidChange = function(
    ...
    if (majorVersion >= 13) {
      ...
      if (
        ...
        owner.notify({
          eventName:
            view_helper_1.ios.traitCollectionColorAppearanceChangedEvent,
          object: owner
        });
      }
    }
...
var iOSFrame = (function() {
  function iOSFrame(frame) {
    ...
    this._controller = UINavigationControllerImpl.initWithOwner(
      new WeakRef(frame)
    );
  }
