...
function _setAndroidFragmentTransitions(
  animated,
  ...
  if (navigationTransition) {
    ...
    name = navigationTransition.name
      ? navigationTransition.name.toLowerCase()
      : '';
  }
  if (!animated) {
    name = 'none';
    ...
    name.indexOf('slide') !== 0 &&
    name !== 'fade' &&
    name.indexOf('flip') !== 0 &&
    name.indexOf('explode') !== 0
  ) {
    name = 'default';
  }
  ...
  if (name === 'none') {
    ...
    setupNewFragmentCustomTransition(
      { duration: transition.getDuration(), curve: transition.getCurve() },
      newEntry,
      transition
    );
    if (currentFragmentNeedsDifferentAnimation) {
    ...
    if (currentFragmentNeedsDifferentAnimation) {
      setupCurrentFragmentFadeTransition(
        { duration: 150, curve: null },
        currentEntry
      );
    }
    ...
    if (currentFragmentNeedsDifferentAnimation) {
      setupCurrentFragmentSlideTransition(
        navigationTransition,
        currentEntry,
        name
      );
    ...
    var flipTransition = new flip_transition_1.FlipTransition(
      direction,
      navigationTransition.duration,
      navigationTransition.curve
    );
    setupNewFragmentCustomTransition(
      navigationTransition,
      newEntry,
...
function setupAllAnimation(entry, transition) {
  ...
  var enterAnimator = transition.createAndroidAnimator(
    transition_1.AndroidTransitionType.enter
  );
  ...
  var popExitAnimator = transition.createAndroidAnimator(
    transition_1.AndroidTransitionType.popExit
  );
...
function setupExitAndPopEnterAnimation(entry, transition) {
  ...
  var exitAnimator = transition.createAndroidAnimator(
    transition_1.AndroidTransitionType.exit
  );
  ...
  var popEnterAnimator = transition.createAndroidAnimator(
    transition_1.AndroidTransitionType.popEnter
  );
    ...
    var AnimationListenerImpl = (function(_super) {
      ...
      AnimationListenerImpl.prototype.onAnimationStart = function(animator) {
        ...
        if (trace_1.isEnabled()) {
          trace_1.write(
            'START ' + animator.transitionType + ' for ' + entry.fragmentTag,
            trace_1.categories.Transition
          );
        }
      };
...
function getTransitionListener(entry, transition) {
  if (!TransitionListener) {
    var TransitionListenerImpl = (function(_super) {
      ...
      TransitionListenerImpl.prototype.onTransitionStart = function(
        transition
      ) {
      ...
      TransitionListenerImpl.prototype.onTransitionResume = function(
        transition
      ) {
        if (trace_1.isEnabled()) {
      ...
      TransitionListenerImpl.prototype.onTransitionPause = function(
        transition
      ) {
        if (trace_1.isEnabled()) {
          trace_1.write(
            'PAUSE ' +
              toShortString(transition) +
              ' transition for ' +
              this.entry.fragmentTag,
            trace_1.categories.Transition
          );
        }
      };
      TransitionListenerImpl.prototype.onTransitionCancel = function(
        transition
      ) {
        if (trace_1.isEnabled()) {
          trace_1.write(
  ...
  if (exitListener) {
    ...
    if (exitTransition) {
      ...
      if (trace_1.isEnabled()) {
        trace_1.write(
          'Cleared Exit ' +
            exitTransition.getClass().getSimpleName() +
            ' transition for ' +
            fragment,
          trace_1.categories.Transition
        );
  ...
  if (reenterListener) {
    ...
    if (reenterTransition) {
      ...
      if (trace_1.isEnabled()) {
        trace_1.write(
          'Cleared Reenter ' +
            reenterTransition.getClass().getSimpleName() +
            ' transition for ' +
            fragment,
          trace_1.categories.Transition
        );
  ...
  if (enterListener) {
    ...
    if (enterTransition) {
      ...
      if (trace_1.isEnabled()) {
        trace_1.write(
          'Cleared Enter ' +
            enterTransition.getClass().getSimpleName() +
            ' transition for ' +
            fragment,
          trace_1.categories.Transition
        );
  ...
  if (returnListener) {
    ...
    if (returnTransition) {
      ...
      if (trace_1.isEnabled()) {
        trace_1.write(
          'Cleared Return ' +
            returnTransition.getClass().getSimpleName() +
            ' transition for ' +
            fragment,
          trace_1.categories.Transition
        );
  ...
  switch (direction) {
    case 'left':
      setEnterTransition(
        navTransition,
        entry,
        new androidx.transition.Slide(android.view.Gravity.RIGHT)
      );
      setReturnTransition(
        navTransition,
        entry,
    ...
    case 'right':
      setEnterTransition(
        navTransition,
        entry,
        new androidx.transition.Slide(android.view.Gravity.LEFT)
      );
      setReturnTransition(
        navTransition,
        entry,
    ...
    case 'top':
      setEnterTransition(
        navTransition,
        entry,
        new androidx.transition.Slide(android.view.Gravity.BOTTOM)
      );
      setReturnTransition(
        navTransition,
        entry,
    ...
    case 'bottom':
      setEnterTransition(
        navTransition,
        entry,
        new androidx.transition.Slide(android.view.Gravity.TOP)
      );
      setReturnTransition(
        navTransition,
        entry,
...
function setupCurrentFragmentCustomTransition(
  navTransition,
  entry,
  transition
) {
  ...
  var exitTransition = new org.nativescript.widgets.CustomTransition(
    exitAnimator,
    transition.constructor.name +
      transition_1.AndroidTransitionType.exit.toString()
  );
  setExitTransition(navTransition, entry, exitTransition);
  var reenterAnimator = transition.createAndroidAnimator(
    transition_1.AndroidTransitionType.popEnter
  );
  var reenterTransition = new org.nativescript.widgets.CustomTransition(
    reenterAnimator,
    transition.constructor.name +
      transition_1.AndroidTransitionType.popEnter.toString()
  );
...
function setupNewFragmentCustomTransition(navTransition, entry, transition) {
  ...
  var enterTransition = new org.nativescript.widgets.CustomTransition(
    enterAnimator,
    transition.constructor.name +
      transition_1.AndroidTransitionType.enter.toString()
  );
  setEnterTransition(navTransition, entry, enterTransition);
  var returnAnimator = transition.createAndroidAnimator(
    transition_1.AndroidTransitionType.popExit
  );
  var returnTransition = new org.nativescript.widgets.CustomTransition(
    returnAnimator,
    transition.constructor.name +
      transition_1.AndroidTransitionType.popExit.toString()
  );
...
function setupNewFragmentFadeTransition(navTransition, entry) {
  ...
  var fadeOutReturn = new androidx.transition.Fade(
    androidx.transition.Fade.OUT
  );
...
function setUpNativeTransition(navigationTransition, nativeTransition) {
  ...
  var interpolator = navigationTransition.curve
    ? animation_1._resolveAnimationCurve(navigationTransition.curve)
    : defaultInterpolator();
...
function transitionOrAnimationCompleted(entry) {
  ...
  if (entries.size === 0) {
    ...
    var navigationContext_1 = frame_1._executingContext || {
      navigationType: frame_common_1.NavigationType.back
    };
    var current_1 = frame_1.isCurrent(entry)
      ? previousCompletedAnimationEntry
      : entry;
    current_1 = current_1 || entry;
    if (current_1) {
      setTimeout(function() {
        return frame_1.setCurrent(
          current_1,
          navigationContext_1.navigationType
        );
      });
    }
  } else {
...
function toShortString(nativeTransition) {
  return (
    nativeTransition.getClass().getSimpleName() +
    '@' +
    nativeTransition.hashCode().toString(16)
  );
}
  ...
  if (entry && trace_1.isEnabled()) {
    ...
    result +=
      ...
      (fragment.getEnterTransition()
        ? ' enter=' + toShortString(fragment.getEnterTransition())
        : '');
    result +=
      '' +
      (fragment.getExitTransition()
        ? ' exit=' + toShortString(fragment.getExitTransition())
        : '');
    result +=
      '' +
      (fragment.getReenterTransition()
        ? ' popEnter=' + toShortString(fragment.getReenterTransition())
        : '');
    result +=
      '' +
      (fragment.getReturnTransition()
        ? ' popExit=' + toShortString(fragment.getReturnTransition())
        : '');
...
function createDummyZeroDurationAnimator(duration) {
  ...
  values[0] = 0.0;
  values[1] = 1.0;
  var animator = android.animation.ObjectAnimator.ofFloat(
    null,
    'alpha',
    values
  );
