...
var UNSPECIFIED = action_bar_common_1.layout.makeMeasureSpec(
  0,
  action_bar_common_1.layout.UNSPECIFIED
);
function loadActionIcon(item) {
  ...
  if (utils_1.isFontIconURI(itemIcon)) {
    var fontIconCode = itemIcon.split('//')[1];
    var font = itemStyle.fontInternal;
    ...
    is = image_source_1.ImageSource.fromFileOrResourceSync(itemIcon);
  }
  if (is && is.ios) {
    img = is.ios;
  } else {
    action_bar_common_1.traceMissingIcon(itemIcon);
  }
  return img;
}
...
var ActionBar = (function(_super) {
  ...
  ActionBar.prototype.layoutInternal = function() {
    var _a = this._getActualSize,
      width = _a.width,
      height = _a.height;
    var widthSpec = action_bar_common_1.layout.makeMeasureSpec(
      width,
      action_bar_common_1.layout.EXACTLY
    );
    var heightSpec = action_bar_common_1.layout.makeMeasureSpec(
      height,
      action_bar_common_1.layout.EXACTLY
    );
  ...
  ActionBar.prototype.update = function() {
    ...
    var indexOfViewController = navController.viewControllers.indexOfObject(
      viewController
    );
    if (
      indexOfViewController > 0 &&
      indexOfViewController < navController.viewControllers.count
    ) {
      previousController =
        navController.viewControllers[indexOfViewController - 1];
    }
    if (previousController) {
      if (this.navigationButton) {
        var tapHandler = TapBarItemHandlerImpl.initWithOwner(
          new WeakRef(this.navigationButton)
        );
        var barButtonItem = UIBarButtonItem.alloc().initWithTitleStyleTargetAction(
          this.navigationButton.text + '',
          0,
          tapHandler,
          'tap'
        );
    ...
    if (
      ...
      action_bar_common_1.isVisible(this.navigationButton) &&
      this.navigationButton.icon
    ) {
    ...
    if (this.navigationButton) {
      ...
      navigationItem.setHidesBackButtonAnimated(
        !action_bar_common_1.isVisible(this.navigationButton),
        false
      );
    }
  ...
  ActionBar.prototype.populateMenuItems = function(navigationItem) {
    ...
    var leftBarItems = NSMutableArray.new();
    var rightBarItems = NSMutableArray.new();
  ...
  ActionBar.prototype.createBarButtonItem = function(item) {
    ...
    if (item.actionView && item.actionView.ios) {
      var recognizer = UITapGestureRecognizer.alloc().initWithTargetAction(
        tapHandler,
        'tap'
      );
      item.actionView.ios.addGestureRecognizer(recognizer);
      barButtonItem = UIBarButtonItem.alloc().initWithCustomView(
        item.actionView.ios
      );
      ...
      barButtonItem = UIBarButtonItem.alloc().initWithTitleStyleTargetAction(
        item.text + '',
        0,
        tapHandler,
        'tap'
  ...
  ActionBar.prototype.setColor = function(navBar, color) {
    ...
    if (color) {
      navBar.titleTextAttributes =
        ((_a = {}), (_a[NSForegroundColorAttributeName] = color.ios), _a);
      navBar.largeTitleTextAttributes =
        ((_b = {}), (_b[NSForegroundColorAttributeName] = color.ios), _b);
  ...
  ActionBar.prototype.onMeasure = function(
    widthMeasureSpec,
    heightMeasureSpec
  ) {
    var _this = this;
    var width = action_bar_common_1.layout.getMeasureSpecSize(widthMeasureSpec);
    var height = action_bar_common_1.layout.getMeasureSpecSize(
      heightMeasureSpec
    );
    if (this.titleView) {
      action_bar_common_1.View.measureChild(
        this,
        this.titleView,
        UNSPECIFIED,
        UNSPECIFIED
  ...
  ActionBar.prototype.onLayout = function(left, top, right, bottom) {
    ...
    if (titleView) {
      if (majorVersion > 10) {
        action_bar_common_1.View.layoutChild(
          this,
          titleView,
          0,
          0,
  ...
  ActionBar.prototype[
    action_bar_common_1.colorProperty.getDefault
  ] = function() {
    return null;
  };
  ActionBar.prototype[action_bar_common_1.colorProperty.setNative] = function(
    color
  ) {
  ...
  ActionBar.prototype[
    action_bar_common_1.backgroundColorProperty.getDefault
  ] = function() {
    return null;
  };
  ActionBar.prototype[
    action_bar_common_1.backgroundColorProperty.setNative
  ] = function(value) {
    var navBar = this.navBar;
    if (navBar) {
      var color =
        value instanceof action_bar_common_1.Color ? value.ios : value;
      navBar.barTintColor = color;
    }
  };
  ActionBar.prototype[
    action_bar_common_1.backgroundInternalProperty.getDefault
  ] = function() {
    return null;
  };
  ActionBar.prototype[
    action_bar_common_1.backgroundInternalProperty.setNative
  ] = function(value) {};
  ActionBar.prototype[action_bar_common_1.flatProperty.setNative] = function(
    value
  ) {
    var navBar = this.navBar;
    if (navBar) {
  ...
  ActionBar.prototype[
    action_bar_common_1.iosIconRenderingModeProperty.getDefault
  ] = function() {
    return 'alwaysOriginal';
  };
  ActionBar.prototype[
    action_bar_common_1.iosIconRenderingModeProperty.setNative
  ] = function(value) {
    this.update();
  };
  return ActionBar;
