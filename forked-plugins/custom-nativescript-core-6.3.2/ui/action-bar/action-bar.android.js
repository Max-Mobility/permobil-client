...
function loadActionIconDrawableOrResourceId(item) {
  ...
  if (utils_1.isFontIconURI(itemIcon)) {
    ...
    var is = image_source_1.ImageSource.fromFontIconCodeSync(
      fontIconCode,
      font,
      color
    );
...
var ActionBar = (function(_super) {
  ...
  ActionBar.prototype._updateNavigationButton = function() {
    ...
    if (navButton && action_bar_common_1.isVisible(navButton)) {
      ...
      if (systemIcon !== undefined) {
        ...
        var drawableOrId = loadActionIconDrawableOrResourceId(navButton);
        if (drawableOrId) {
          this.nativeViewProtected.setNavigationIcon(drawableOrId);
        }
      }
      this.nativeViewProtected.setNavigationContentDescription(
        navButton.text || null
      );
  ...
  ActionBar.prototype._updateIcon = function() {
    ...
    if (visibility) {
      ...
      if (icon !== undefined) {
        ...
        var defaultIcon = application.android.nativeApp.getApplicationInfo()
          .icon;
  ...
  ActionBar.prototype._addActionItems = function() {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (item.actionView && item.actionView.android) {
        ...
        var drawableOrId = loadActionIconDrawableOrResourceId(item);
        if (drawableOrId) {
  ...
  ActionBar.prototype[action_bar_common_1.colorProperty.setNative] = function(
    ...
    var color =
      value instanceof action_bar_common_1.Color ? value.android : value;
  ...
  ActionBar.prototype[action_bar_common_1.flatProperty.setNative] = function(
    ...
    if (compat.setElevation) {
      if (value) {
        ...
        var val =
          DEFAULT_ELEVATION * action_bar_common_1.layout.getDisplayDensity();
        compat.setElevation(this.nativeViewProtected, val);
      }
    }
  };
  ActionBar.prototype[
    action_bar_common_1.androidContentInsetLeftProperty.setNative
  ] = function() {
    if (apiLevel >= 21) {
  ...
  ActionBar.prototype[
    action_bar_common_1.androidContentInsetRightProperty.setNative
  ] = function() {
    if (apiLevel >= 21) {
      this.nativeViewProtected.setContentInsetsAbsolute(
        this.effectiveContentInsetLeft,
...
function getDrawableOrResourceId(icon, resources) {
  ...
  if (icon.indexOf(utils_1.RESOURCE_PREFIX) === 0) {
    var resourceId = resources.getIdentifier(
      icon.substr(utils_1.RESOURCE_PREFIX.length),
      'drawable',
    ...
    var drawable = void 0;
    var is = image_source_1.ImageSource.fromFileOrResourceSync(icon);
    if (is) {
