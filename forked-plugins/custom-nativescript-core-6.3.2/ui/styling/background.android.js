...
(function(ad) {
  ...
  function onBackgroundOrBorderPropertyChanged(view) {
    ...
    if (
      isSetColorFilterOnlyWidget(nativeView) &&
      drawable &&
      ...
      if (drawable instanceof org.nativescript.widgets.BorderDrawable) {
        if (
          !(
            androidView._cachedDrawable instanceof
            android.graphics.drawable.Drawable.ConstantState
          )
        ) {
          return;
        }
        drawable = androidView._cachedDrawable.newDrawable(
          nativeView.getResources()
        );
        nativeView.setBackground(drawable);
      }
      var backgroundColor = (drawable.backgroundColor =
    ...
    var leftPadding = Math.ceil(
      view.effectiveBorderLeftWidth + view.effectivePaddingLeft
    );
    var topPadding = Math.ceil(
      view.effectiveBorderTopWidth + view.effectivePaddingTop
    );
    var rightPadding = Math.ceil(
      view.effectiveBorderRightWidth + view.effectivePaddingRight
    );
    var bottomPadding = Math.ceil(
      view.effectiveBorderBottomWidth + view.effectivePaddingBottom
    );
...
function refreshBorderDrawable(view, borderDrawable) {
  ...
  var background = view.style.backgroundInternal;
  if (background) {
    var backgroundPositionParsedCSSValues = createNativeCSSValueArray(
      background.position
    );
    var backgroundSizeParsedCSSValues = createNativeCSSValueArray(
      background.size
    );
    ...
    if (utils_1.isDataURI(imageUri)) {
      ...
      if (imageUri.indexOf(utils_1.RESOURCE_PREFIX) !== 0) {
        ...
        if (fileName.indexOf('~/') === 0) {
          fileName = file_system_1.path.join(
            file_system_1.knownFolders.currentApp().path,
            fileName.replace('~/', '')
          );
        }
    ...
    borderDrawable.refresh(
      background.borderTopColor
        ? background.borderTopColor.android
        : blackColor,
      background.borderRightColor
        ? background.borderRightColor.android
        : blackColor,
      background.borderBottomColor
        ? background.borderBottomColor.android
        : blackColor,
      background.borderLeftColor
        ? background.borderLeftColor.android
        : blackColor,
...
function createNativeCSSValueArray(css) {
  ...
  var nativeArray = Array.create(
    org.nativescript.widgets.CSSValue,
    cssValues.length
  );
...
application.android.on(
  'activityStarted',
  profiling_1.profile('initImageCache', function(args) {
    if (!imageFetcher) {
...
application.android.on(
  'activityStopped',
  profiling_1.profile('closeImageCache', function(args) {
    if (imageFetcher) {
