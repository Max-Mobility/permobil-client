...
var ANIMATION_PROPERTY_HANDLERS = Object.freeze({
  ...
  'animation-iteration-count': function(info, value) {
    return (info.iterations =
      value === 'infinite' ? Number.MAX_VALUE : parseFloat(value));
  },
...
var CssAnimationParser = (function() {
  ...
  CssAnimationParser.keyframeAnimationsFromCSSDeclarations = function(
    declarations
  ) {
      ...
      for (var _a = 0, _b = keyframe.values; _a < _b.length; _a++) {
        ...
        for (var _c = 0, _d = keyframe.declarations; _c < _d.length; _c++) {
          ...
          if (declaration.property === 'animation-timing-function') {
            current.curve = converters_1.animationTimingFunctionConverter(
              declaration.value
            );
          }
        }
...
function keyframeAnimationsFromCSSProperty(value, animations) {
  if (typeof value === 'string') {
    ...
    for (var _i = 0, values_1 = values; _i < values_1.length; _i++) {
      ...
      if (arr.length > 2) {
        animationInfo.curve = converters_1.animationTimingFunctionConverter(
          arr[2]
        );
      }
      if (arr.length > 3) {
...
function parseKeyframeDeclarations(unparsedKeyframeDeclarations) {
  var declarations = unparsedKeyframeDeclarations.reduce(function(
    declarations,
    _a
  ) {
    var unparsedProperty = _a.property,
      unparsedValue = _a.value;
    var property = properties_1.CssAnimationProperty._getByCssName(
      unparsedProperty
    );
    if (
      typeof unparsedProperty === 'string' &&
      property &&
      property._valueConverter
    ) {
      declarations[property.name] = property._valueConverter(unparsedValue);
    } else if (
      typeof unparsedValue === 'string' &&
      unparsedProperty === 'transform'
    ) {
      var transformations = style_properties_1.transformConverter(
        unparsedValue
      );
  ...
  return Object.keys(declarations).map(function(property) {
    return { property: property, value: declarations[property] };
  });
}
