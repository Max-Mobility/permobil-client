...
function evaluateCssExpressions(view, property, value) {
  var newValue = properties_1._evaluateCssVariableExpression(
    view,
    property,
    value
  );
  ...
  try {
    ...
    trace_1.write(
      'Failed to evaluate css-calc for property [' +
        property +
        '] for expression [' +
        value +
        '] to ' +
...
function mergeCssSelectors() {
  ...
  applicationCssSelectors.push.apply(
    applicationCssSelectors,
    applicationAdditionalSelectors
  );
...
var CSSSource = (function() {
  ...
  CSSSource.fromURI = function(uri, keyframes) {
    ...
    var sanitizedModuleName = module_name_sanitizer_1.sanitizeModuleName(
      appRelativeUri
    );
    var resolvedModuleName = module_name_resolver_1.resolveModuleName(
      sanitizedModuleName,
      'css'
    );
    try {
  ...
  CSSSource.resolveCSSPathFromURL = function(url, importSource) {
    ...
    var file = resolveFileNameFromUrl(
      url,
      app,
      file_system_1.File.exists,
      importSource
    );
    return file;
  };
  ...
  CSSSource.prototype.createSelectors = function() {
    if (this._ast) {
      this._selectors = this.createSelectorsFromImports().concat(
...
function addTaggedAdditionalCSS(cssText, tag) {
  var parsed = CSSSource.fromSource(cssText, applicationKeyframes, undefined)
    .selectors;
  var changed = false;
  if (parsed && parsed.length) {
    ...
    applicationAdditionalSelectors.push.apply(
      applicationAdditionalSelectors,
      parsed
    );
    mergeCssSelectors();
...
var onCssChanged = profiling_1.profile('"style-scope".onCssChanged', function(
  args
) {
  if (args.cssText) {
    var parsed = CSSSource.fromSource(
      args.cssText,
...
exports.loadAppCSS = profiling_1.profile('"style-scope".loadAppCSS', function(
  args
) {
  loadCss(args.cssFile);
...
var CssState = (function() {
  ...
  CssState.prototype.isSelectorsLatestVersionApplied = function() {
    ...
    return (
      this.viewRef.get()._styleScope._getSelectorsVersion() ===
  ...
  CssState.prototype.updateDynamicState = function() {
    ...
    var matchingSelectors = this._match.selectors.filter(function(sel) {
      return sel.dynamic ? sel.match(view) : true;
    });
  ...
  CssState.prototype.playKeyframeAnimations = function(matchingSelectors) {
    ...
    matchingSelectors.forEach(function(selector) {
      ...
      if (ruleAnimations) {
        ...
        for (
          var _i = 0, ruleAnimations_1 = ruleAnimations;
          _i < ruleAnimations_1.length;
          _i++
        ) {
          var animationInfo = ruleAnimations_1[_i];
          var animation = keyframeAnimationModule.KeyframeAnimation.keyframeAnimationFromInfo(
            animationInfo
          );
          if (animation) {
    ...
    if ((this._playsKeyframeAnimations = animations.length > 0)) {
      ...
      if (!view_1) {
        trace_1.write(
          ...
          trace_1.categories.Animation,
          trace_1.messageType.warn
        );
        return;
      }
      animations.map(function(animation) {
        return animation.play(view_1);
      });
  ...
  CssState.prototype.setPropertyValues = function(matchingSelectors) {
    ...
    if (!view) {
      trace_1.write(
        matchingSelectors +
          ' not set to view\'s property because ".viewRef" is cleared',
        trace_1.categories.Style,
        trace_1.messageType.warn
      );
      return;
    ...
    for (var property in newPropertyValues) {
      ...
      isCssExpressionInUse =
        isCssExpressionInUse ||
        properties_1.isCssVariableExpression(value) ||
        properties_1.isCssCalcExpression(value);
    }
    if (isCssExpressionInUse) {
      for (var property in newPropertyValues) {
        var value = evaluateCssExpressions(
          view,
          property,
          newPropertyValues[property]
        );
    ...
    for (var property in newPropertyValues) {
      if (
        oldProperties &&
        property in oldProperties &&
        oldProperties[property] === newPropertyValues[property]
      ) {
        continue;
      }
      var value = newPropertyValues[property];
      ...
      if (changes.attributes) {
        changes.attributes.forEach(function(attribute) {
          view.addEventListener(
            attribute + 'Change',
            _this._onDynamicStateChangeHandler
          );
        });
      }
...
var StyleScope = (function() {
  ...
  StyleScope.prototype.getKeyframeAnimationWithName = function(animationName) {
    ...
    ensureCssAnimationParserModule();
    animation.keyframes = cssAnimationParserModule.CssAnimationParser.keyframesArrayFromCSS(
  ...
  StyleScope.prototype.isApplicationCssSelectorsLatestVersionApplied = function() {
    return (
      this._applicationCssSelectorsAppliedVersion ===
      applicationCssSelectorVersion
    );
  };
  ...
  StyleScope.prototype._createSelectors = function() {
    ...
    if (toMerge.length > 0) {
      ...
      this._selectors = new css_selector_1.SelectorsMap(
        this._mergedCssSelectors
      );
    }
  };
...
function resolveFileNameFromUrl(url, appDirectory, fileExists, importSource) {
  ...
  var absolutePath = isAbsolutePath
    ? fileName
    : file_system_1.path.join(appDirectory, fileName);
  ...
  if (!isAbsolutePath) {
    ...
    var external_1 = file_system_1.path.join(
      appDirectory,
      'tns_modules',
      fileName
    );
...
function resolveFilePathFromImport(importSource, fileName) {
  ...
  var fileNameParts = fileName
    .split(file_system_1.path.separator)
    .filter(function(p) {
      return !isCurrentDirectory(p);
    });
  importSourceParts.pop();
  fileNameParts.forEach(function(p) {
    return isParentDirectory(p)
      ? importSourceParts.pop()
      : importSourceParts.push(p);
  });
  return importSourceParts.join(file_system_1.path.separator);
}
exports.applyInlineStyle = profiling_1.profile(function applyInlineStyle(
  view,
  styleStr
) {
