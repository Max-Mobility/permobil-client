...
var Font = (function(_super) {
  ...
  Font.default = new Font(
    undefined,
    undefined,
    font_common_1.FontStyle.NORMAL,
    font_common_1.FontWeight.NORMAL
  );
  return Font;
})(font_common_1.Font);
exports.Font = Font;
...
function getSystemFont(size, nativeWeight, italic, symbolicTraits) {
  ...
  if (italic) {
    var descriptor = result.fontDescriptor.fontDescriptorWithSymbolicTraits(
      symbolicTraits
    );
...
function createUIFont(font, defaultFont) {
  ...
  var fontDescriptorTraits =
    ((_a = {}), (_a[UIFontSymbolicTrait] = symbolicTraits), _a);
  ...
  for (var i = 0; i < fontFamilies.length; i++) {
    ...
    if (shouldUseSystemFont(fontFamily)) {
      ...
      var fontAttributes =
        ...
        _b);
      var descriptor = UIFontDescriptor.fontDescriptorWithFontAttributes(
        fontAttributes
      );
      result = UIFont.fontWithDescriptorSize(descriptor, size);
  ...
  function registerFont(fontFile) {
    ...
    if (!CTFontManagerRegisterGraphicsFont(font, error)) {
      if (trace_1.isEnabled()) {
        trace_1.write(
          'Error occur while registering font: ' +
            CFErrorCopyDescription(error.value),
