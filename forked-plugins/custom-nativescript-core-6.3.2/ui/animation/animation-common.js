  ...
  function AnimationBase(animationDefinitions, playSequentially) {
    ...
    for (var i = 0, length_1 = animationDefinitions.length; i < length_1; i++) {
      if (animationDefinitions[i].curve) {
        animationDefinitions[i].curve = this._resolveAnimationCurve(
          animationDefinitions[i].curve
        );
      }
      this._propertyAnimations = this._propertyAnimations.concat(
        AnimationBase._createPropertyAnimations(animationDefinitions[i])
      );
    }
    ...
    if (trace_1.isEnabled()) {
      trace_1.write(
        'Created ' +
          this._propertyAnimations.length +
  ...
  AnimationBase._createPropertyAnimations = function(animationDefinition) {
    ...
    for (var item in animationDefinition) {
      ...
      if (
        (item === Properties.opacity ||
          ...
          item === 'delay' ||
          item === 'iterations') &&
        typeof animationDefinition[item] !== 'number'
      ) {
        throw new Error(
          'Property ' +
            item +
            ' must be valid number. Value: ' +
            animationDefinition[item]
        );
      } else if (
        (item === Properties.scale || item === Properties.translate) &&
        (typeof animationDefinition[item].x !== 'number' ||
          typeof animationDefinition[item].y !== 'number')
      ) {
        throw new Error(
          'Property ' +
            item +
            ' must be valid Pair. Value: ' +
            animationDefinition[item]
        );
      } else if (
        item === Properties.backgroundColor &&
        !color_1.Color.isValid(animationDefinition.backgroundColor)
      ) {
        throw new Error(
          'Property ' +
            item +
            ' must be valid color. Value: ' +
            animationDefinition[item]
        );
      } else if (item === Properties.width || item === Properties.height) {
        animationDefinition[item] = style_properties_1.PercentLength.parse(
          animationDefinition[item]
        );
      }
    }
    ...
    if (animationDefinition.backgroundColor !== undefined) {
      propertyAnimations.push({
        ...
        value:
          typeof animationDefinition.backgroundColor === 'string'
            ? new color_1.Color(animationDefinition.backgroundColor)
