...
var KeyframeAnimation = (function() {
  ...
  KeyframeAnimation.keyframeAnimationFromInfo = function(info) {
    ...
    if (info.isReverse) {
      for (var index_1 = length - 1; index_1 >= 0; index_1--) {
        ...
        startDuration = KeyframeAnimation.parseKeyframe(
          info,
          keyframe,
          animations,
          startDuration
    ...
    animations.map(function(a) {
      return a['curve'] ? a : Object.assign(a, { curve: info.curve });
    });
  ...
  KeyframeAnimation.parseKeyframe = function(
    info,
    keyframe,
    animations,
    startDuration
  ...
  KeyframeAnimation.prototype.cancel = function() {
    if (!this.isPlaying) {
      trace_1.write(
        'Keyframe animation is already playing.',
  ...
  KeyframeAnimation.prototype.animate = function(view, index, iterations) {
    ...
    if (index === 0) {
      ...
      if ('backgroundColor' in animation) {
        view.style[style_properties_1.backgroundColorProperty.keyframe] =
          animation.backgroundColor;
      }
      if ('scale' in animation) {
        view.style[style_properties_1.scaleXProperty.keyframe] =
          animation.scale.x;
        view.style[style_properties_1.scaleYProperty.keyframe] =
          animation.scale.y;
      }
      if ('translate' in animation) {
        view.style[style_properties_1.translateXProperty.keyframe] =
          animation.translate.x;
        view.style[style_properties_1.translateYProperty.keyframe] =
          animation.translate.y;
      }
      if ('rotate' in animation) {
        view.style[style_properties_1.rotateProperty.keyframe] =
          animation.rotate;
      }
      if ('opacity' in animation) {
        view.style[style_properties_1.opacityProperty.keyframe] =
          animation.opacity;
      }
      if ('height' in animation) {
        view.style[style_properties_1.heightProperty.keyframe] =
          animation.height;
      }
  ...
  KeyframeAnimation.prototype._resetAnimationValues = function(
    view,
    animation
  ) {
    if ('backgroundColor' in animation) {
    ...
    if ('width' in animation) {
      view.style[style_properties_1.widthProperty.keyframe] =
        properties_1.unsetValue;
    }
  };
