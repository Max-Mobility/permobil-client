...
(function(bindingConstants) {
  ...
  (bindingConstants =
    exports.bindingConstants || (exports.bindingConstants = {}))
);
...
function isNamedParam(value) {
  ...
  if (equalSignIndex > -1) {
    var equalSignSurround = value.substr(
      equalSignIndex > 0 ? equalSignIndex - 1 : 0,
      3
    );
...
function parseNamedProperties(parameterList, knownOptions, callback) {
  ...
  for (var i = 0; i < parameterList.length; i++) {
    nameValuePair = getPropertyNameValuePair(
      parameterList[i],
      knownOptions,
      callback
    );
    if (nameValuePair) {
      result[nameValuePair[namedParamConstants.propName]] =
        nameValuePair[namedParamConstants.propValue];
    }
  }
  return result;
}
...
function isExpression(expression) {
  if (expression.search(expressionSymbolsRegex) > -1) {
    ...
    if (parentsMatches) {
      var restOfExpression = expression.substr(
        expression.indexOf(parentsMatches[0]) + parentsMatches[0].length
      );
...
function getBindingOptions(name, value) {
  ...
  if (!areNamedParams(params)) {
    if (params.length === 1) {
      ...
      namedParams.push(
        bindingConstants.sourceProperty + ' = ' + params[0].trim()
      );
  ...
  var bindingOptionsParameters = parseNamedProperties(
    namedParams,
    xmlBindingProperties,
    bindingPropertyHandler
  );
  var bindOptions = {
    targetProperty: name
  };
