...
var Builder = (function() {
  function Builder() {}
  Builder.createViewFromEntry = function(entry) {
    if (entry.create) {
      ...
      var moduleName = module_name_sanitizer_1.sanitizeModuleName(
        entry.moduleName
      );
      var resolvedCodeModuleName = module_name_resolver_1.resolveModuleName(
        moduleName,
        ''
      );
      var moduleExports = resolvedCodeModuleName
        ? global.loadModule(resolvedCodeModuleName, true)
        : null;
      if (moduleExports && moduleExports.createPage) {
        var view = moduleExports.createPage();
        var resolvedCssModuleName = module_name_resolver_1.resolveModuleName(
          moduleName,
          'css'
        );
    ...
    throw new Error(
      'Failed to load page XML file for module: ' + entry.moduleName
    );
  };
  Builder.parse = function(value, context) {
  ...
  Builder.load = function(pathOrOptions, context) {
    var componentModule;
    if (typeof pathOrOptions === 'string') {
      ...
      componentModule = loadCustomComponent(
        pathOrOptions.path,
        pathOrOptions.name,
        pathOrOptions.attributes,
        pathOrOptions.exports,
        pathOrOptions.page,
        true
      );
    }
  ...
  Builder.parseMultipleTemplates = function(value, context) {
    var dummyComponent =
      '<ListView><ListView.itemTemplates>' +
      value +
      '</ListView.itemTemplates></ListView>';
...
function parseMultipleTemplates(value, context) {
  console.log(
    'parseMultipleTemplates() is deprecated. Use Builder.parseMultipleTemplates() instead.'
  );
...
function createViewFromEntry(entry) {
  console.log(
    'createViewFromEntry() is deprecated. Use Builder.createViewFromEntry() instead.'
  );
  return Builder.createViewFromEntry(entry);
}
exports.createViewFromEntry = createViewFromEntry;
function loadInternal(moduleName, moduleExports) {
  var componentModule;
  var resolvedXmlModule = module_name_resolver_1.resolveModuleName(
    moduleName,
    'xml'
  );
  if (resolvedXmlModule) {
    var text = global.loadModule(resolvedXmlModule, true);
    componentModule = parseInternal(
      text,
      moduleExports,
      resolvedXmlModule,
      moduleName
...
function loadCustomComponent(
  ...
  var result;
  componentNamespace = module_name_sanitizer_1.sanitizeModuleName(
    componentNamespace
  );
  ...
  var resolvedXmlModuleName = module_name_resolver_1.resolveModuleName(
    moduleName,
    'xml'
  );
  var resolvedCssModuleName = module_name_resolver_1.resolveModuleName(
  ...
  if (resolvedXmlModuleName) {
    ...
    if (
      types_1.isDefined(result) &&
      types_1.isDefined(result.component) &&
      types_1.isDefined(attributes)
    ) {
      for (var attr in attributes) {
        component_builder_1.setPropertyValue(
          result.component,
          subExports,
          context,
          attr,
    ...
    result = component_builder_1.getComponentModule(
      componentName,
      componentNamespace,
      attributes,
      context,
    ...
    if (!resolvedCssModuleName) {
      resolvedCssModuleName = module_name_resolver_1.resolveModuleName(
        componentNamespace,
        'css'
      );
    }
...
function parseInternal(value, context, xmlModule, moduleName) {
  ...
  var ui;
  var errorFormat =
    debug_1.debug && xmlModule
      ? xml2ui.SourceErrorFormat(xmlModule)
      : xml2ui.PositionErrorFormat;
  var componentSourceTracker =
    debug_1.debug && xmlModule
  ...
  (start = new xml2ui.XmlStringParser(errorFormat))
    ...
    .pipe(
      new xml2ui.XmlStateParser(
        (ui = new xml2ui.ComponentParser(
          context,
          errorFormat,
          componentSourceTracker,
          moduleName
        ))
      )
    );
...
(function(xml2ui) {
  ...
  var XmlStringParser = (function(_super) {
    ...
    XmlStringParser.prototype.parse = function(value) {
      var _this = this;
      var xmlParser = new xml.XmlParser(
        function(args) {
          try {
            _this.next(args);
          } catch (e) {
      ...
      } else if (types_1.isObject(value) && types_1.isString(value.default)) {
  ...
  function PositionErrorFormat(e, p) {
    return new debug_1.ScopeError(
      e,
      'Parsing XML at ' + p.line + ':' + p.column
    );
  }
  ...
  function SourceErrorFormat(uri) {
    return function(e, p) {
      var source = p
        ? new debug_1.Source(uri, p.line, p.column)
        : new debug_1.Source(uri, -1, -1);
  ...
  var PlatformFilter = (function(_super) {
    ...
    PlatformFilter.prototype.parse = function(args) {
          ...
          if (this.currentPlatformContext) {
            throw new Error(
              "Already in '" +
                ...
                "' platform context and cannot switch to '" +
                args.elementName +
                "' platform! Platform tags cannot be nested."
            );
          }
      ...
      if (
        this.currentPlatformContext &&
        !PlatformFilter.isCurentPlatform(this.currentPlatformContext)
      ) {
        return;
      }
      this.next(args);
    ...
    PlatformFilter.isCurentPlatform = function(value) {
      return (
        value && value.toLowerCase() === platform_1.device.os.toLowerCase()
      );
    };
  ...
  var TemplateParser = (function() {
    ...
    TemplateParser.prototype.parseStartElement = function(
      prefix,
      namespace,
      elementName,
      attributes
      ...
      if (this._state === 0) {
        ...
        throw new Error(
          'Template must have exactly one root element but multiple elements were found.'
        );
      }
      this._nestingLevel++;
    };
    TemplateParser.prototype.parseEndElement = function(prefix, elementName) {
      if (this._state === 0) {
        throw new Error(
          'Template must have exactly one root element but none was found.'
        );
      } else if (this._state === 2) {
      ...
      if (this._nestingLevel === 0) {
        this._state = 2;
        if (
          this._setTemplateProperty &&
          this._templateProperty.name in this._templateProperty.parent.component
        ) {
          var template = this.buildTemplate();
          this._templateProperty.parent.component[
            this._templateProperty.name
          ] = template;
        }
      }
    };
    TemplateParser.prototype.buildTemplate = function() {
      ...
      var template = profiling_1.profile('Template()', function() {
        var start;
        var ui;
        (start = new xml2ui.XmlArgsReplay(
          _this._recordedXmlStream,
          errorFormat
        )).pipe(
          new XmlStateParser(
            (ui = new ComponentParser(context, errorFormat, sourceTracker))
          )
        );
        start.replay();
  ...
  var MultiTemplateParser = (function() {
    ...
    MultiTemplateParser.prototype.parse = function(args) {
      if (
        args.eventType === xml.ParserEventType.StartElement &&
        args.elementName === 'template'
      ) {
        var childParser = new TemplateParser(
          this,
  ...
  var ComponentParser = (function() {
    ...
    ComponentParser.prototype.buildComponent = function(args) {
      if (args.prefix && args.namespace) {
        ...
        return component_builder_1.getComponentModule(
          args.elementName,
          namespace,
          args.attributes,
          this.context,
    ...
    ComponentParser.prototype.parse = function(args) {
      ...
      var complexProperty = this.complexProperties[
        this.complexProperties.length - 1
      ];
      if (args.eventType === xml.ParserEventType.StartElement) {
        if (ComponentParser.isComplexProperty(args.elementName)) {
          ...
          if (ComponentParser.isKnownTemplate(name_2, parent.exports)) {
            return new TemplateParser(this, {
              context:
                (parent ? getExports(parent.component) : null) || this.context,
              parent: parent,
              name: name_2,
          ...
          if (componentModule) {
            ...
            if (parent) {
              if (complexProperty) {
                ComponentParser.addToComplexProperty(
                  parent,
                  complexProperty,
                  componentModule
                );
        ...
        if (ComponentParser.isComplexProperty(args.elementName)) {
          if (complexProperty) {
            if (complexProperty.parser) {
              parent.component[complexProperty.name] =
                complexProperty.parser.value;
            } else if (parent && parent.component._addArrayFromBuilder) {
              parent.component._addArrayFromBuilder(
    ...
    ComponentParser.isKnownTemplate = function(name, exports) {
      return (
        ComponentParser.KNOWNTEMPLATES in exports &&
        exports[ComponentParser.KNOWNTEMPLATES] &&
        name in exports[ComponentParser.KNOWNTEMPLATES]
      );
    };
    ComponentParser.isKnownMultiTemplate = function(name, exports) {
      return (
        ComponentParser.KNOWNMULTITEMPLATES in exports &&
        exports[ComponentParser.KNOWNMULTITEMPLATES] &&
        name in exports[ComponentParser.KNOWNMULTITEMPLATES]
      );
    };
    ComponentParser.addToComplexProperty = function(
      parent,
      complexProperty,
      elementModule
    ) {
      ...
      if (
        ComponentParser.isKnownCollection(complexProperty.name, parent.exports)
      ) {
        complexProperty.items.push(elementModule.component);
      } else if (parentComponent._addChildFromBuilder) {
        parentComponent._addChildFromBuilder(
          complexProperty.name,
          elementModule.component
        );
      } else {
    ...
    ComponentParser.isKnownCollection = function(name, context) {
      return (
        ComponentParser.KNOWNCOLLECTIONS in context &&
        context[ComponentParser.KNOWNCOLLECTIONS] &&
        name in context[ComponentParser.KNOWNCOLLECTIONS]
      );
    };
