...
function initializeItemClickListener() {
  ...
  var ItemClickListenerImpl = (function(_super) {
    ...
    ItemClickListenerImpl.prototype.onItemClick = function(
      parent,
      convertView,
      index,
      id
      ...
      var view = owner._realizedTemplates
        .get(owner._getItemTemplate(index).key)
        .get(convertView);
      owner.notify({
        eventName: ITEMTAP,
        object: owner,
...
var ListView = (function(_super) {
  ...
  ListView.prototype[
    ...
    if (value instanceof list_view_common_1.Color) {
      nativeView.setDivider(
        new android.graphics.drawable.ColorDrawable(value.android)
      );
  ...
  var ListViewAdapter = (function(_super) {
    ...
    ListViewAdapter.prototype.getItem = function(i) {
      if (this.owner && this.owner.items && i < this.owner.items.length) {
        ...
        return getItem
          ? getItem.call(this.owner.items, i)
          : this.owner.items[i];
      }
      return null;
    };
    ...
    ListViewAdapter.prototype.getView = function(index, convertView, parent) {
      ...
      if (convertView) {
        ...
        if (!view) {
          throw new Error(
            "There is no entry with key '" +
              convertView +
              "' in the realized views cache for template with key'" +
              template.key +
              "'."
          );
        }
      ...
      if (args.view) {
        ...
        var realizedItemsForTemplateKey = this.owner._realizedTemplates.get(
          template.key
        );
        if (!realizedItemsForTemplateKey) {
          realizedItemsForTemplateKey = new Map();
          this.owner._realizedTemplates.set(
            template.key,
            realizedItemsForTemplateKey
          );
        }
