...
var View = (function(_super) {
  ...
  Object.defineProperty(View.prototype, 'isLayoutRequired', {
    get: function() {
      return (
        (this._privateFlags & PFLAG_LAYOUT_REQUIRED) === PFLAG_LAYOUT_REQUIRED
      );
    },
    enumerable: true,
    configurable: true
  ...
  View.prototype.measure = function(widthMeasureSpec, heightMeasureSpec) {
    var measureSpecsChanged = this._setCurrentMeasureSpecs(
      widthMeasureSpec,
      heightMeasureSpec
    );
    var forceLayout =
      (this._privateFlags & PFLAG_FORCE_LAYOUT) === PFLAG_FORCE_LAYOUT;
  ...
  View.prototype.setMeasuredDimension = function(
    ...
    _super.prototype.setMeasuredDimension.call(
      this,
      measuredWidth,
      measuredHeight
    );
  ...
  View.prototype.onMeasure = function(widthMeasureSpec, heightMeasureSpec) {
    ...
    var nativeHeight = 0;
    if (view) {
      var nativeSize = view_common_1.layout.measureNativeView(
        view,
        width,
        widthMode,
        height,
    ...
    var widthAndState = View.resolveSizeAndState(
      measureWidth,
      width,
      widthMode,
      0
    );
    var heightAndState = View.resolveSizeAndState(
      measureHeight,
      height,
      heightMode,
      0
  ...
  View.prototype._setNativeViewFrame = function(nativeView, frame) {
    ...
    if (!CGRectEqualToRect(oldFrame, frame)) {
      if (view_common_1.traceEnabled()) {
        view_common_1.traceWrite(
          this +
            ' :_setNativeViewFrame: ' +
            JSON.stringify(view_helper_1.ios.getPositionFromFrame(frame)),
          view_common_1.traceCategories.Layout
        );
      }
      ...
      nativeView.bounds = CGRectMake(
        boundsOrigin.x,
        boundsOrigin.y,
        boundsFrame.size.width,
        boundsFrame.size.height
      );
  ...
  View.prototype.layoutNativeView = function(left, top, right, bottom) {
    ...
    var nativeView = this.nativeViewProtected;
    var frame = view_helper_1.ios.getFrameFromPosition({
      left: left,
      top: top,
      right: right,
      bottom: bottom
  ...
  View.prototype._setLayoutFlags = function(left, top, right, bottom) {
    ...
    var widthSpec = view_common_1.layout.makeMeasureSpec(
      width,
      view_common_1.layout.EXACTLY
    );
    var heightSpec = view_common_1.layout.makeMeasureSpec(
      height,
      view_common_1.layout.EXACTLY
    );
    ...
    var sizeChanged = this._setCurrentLayoutBounds(left, top, right, bottom)
      .sizeChanged;
    this.updateBackground(sizeChanged);
    this._privateFlags &= ~PFLAG_LAYOUT_REQUIRED;
  };
  ...
  View.prototype.getSafeAreaInsets = function() {
    var safeAreaInsets =
      this.nativeViewProtected && this.nativeViewProtected.safeAreaInsets;
    var insets = { left: 0, top: 0, right: 0, bottom: 0 };
    if (safeAreaInsets) {
      insets.left = view_common_1.layout.round(
        view_common_1.layout.toDevicePixels(safeAreaInsets.left)
      );
      insets.top = view_common_1.layout.round(
        view_common_1.layout.toDevicePixels(safeAreaInsets.top)
      );
      insets.right = view_common_1.layout.round(
        view_common_1.layout.toDevicePixels(safeAreaInsets.right)
      );
      insets.bottom = view_common_1.layout.round(
        view_common_1.layout.toDevicePixels(safeAreaInsets.bottom)
      );
    }
    return insets;
  };
  View.prototype.getLocationInWindow = function() {
    ...
    var pointInWindow = this.nativeViewProtected.convertPointToView(
      this.nativeViewProtected.bounds.origin,
      null
    );
    return {
      x: pointInWindow.x,
  ...
  View.prototype.getLocationOnScreen = function() {
    ...
    var pointOnScreen = this.nativeViewProtected.window.convertPointToWindow(
      pointInWindow,
      null
    );
    return {
  ...
  View.prototype.getLocationRelativeTo = function(otherView) {
    if (
      ...
      !this.nativeViewProtected.window ||
      !otherView.nativeViewProtected ||
      !otherView.nativeViewProtected.window ||
      this.nativeViewProtected.window !== otherView.nativeViewProtected.window
    ) {
      return undefined;
    }
    var myPointInWindow = this.nativeViewProtected.convertPointToView(
      this.nativeViewProtected.bounds.origin,
      null
    );
    var otherPointInWindow = otherView.nativeViewProtected.convertPointToView(
      otherView.nativeViewProtected.bounds.origin,
      null
    );
    return {
  ...
  View.prototype._onSizeChanged = function() {
    ...
    var background = this.style.backgroundInternal;
    var backgroundDependsOnSize =
      background.image ||
      !background.hasUniformBorder() ||
      background.hasBorderRadius();
    if (
      this._nativeBackgroundState === 'invalid' ||
      (this._nativeBackgroundState === 'drawn' && backgroundDependsOnSize)
    ) {
      this._redrawNativeBackground(background);
    }
    ...
    if (
      clipPath !== '' &&
      this[style_properties_1.clipPathProperty.setNative]
    ) {
  ...
  View.prototype.updateNativeTransform = function() {
    ...
    newTransform = CGAffineTransformTranslate(
      newTransform,
      this.translateX,
      this.translateY
    );
    newTransform = CGAffineTransformRotate(
      newTransform,
      (rotate * Math.PI) / 180
    );
    ...
    if (
      !CGAffineTransformEqualToTransform(
        this.nativeViewProtected.transform,
        newTransform
      )
    ) {
  ...
  View.prototype._showNativeModalView = function(parent, options) {
    var _this = this;
    var parentWithController = view_helper_1.ios.getParentWithViewController(
      parent
    );
    if (!parentWithController) {
      view_common_1.traceWrite(
        'Could not find parent with viewController for ' +
          parent +
          ' while showing modal view.',
        view_common_1.traceCategories.ViewHierarchy,
        view_common_1.traceMessageType.error
      );
      return;
    }
    ...
    if (parentController.presentedViewController) {
      view_common_1.traceWrite(
        'Parent is already presenting view controller. Close the current modal page before showing another one!',
        view_common_1.traceCategories.ViewHierarchy,
        view_common_1.traceMessageType.error
      );
      return;
    ...
    _super.prototype._showNativeModalView.call(
      this,
    ...
    if (!controller) {
      ...
      controller = view_helper_1.ios.UILayoutViewController.initWithOwner(
        new WeakRef(this)
      );
  ...
  View.prototype._hideNativeModalView = function(parent, whenClosedCallback) {
    if (!parent || !parent.viewController) {
      view_common_1.traceError(
        'Trying to hide modal view but no parent with viewController specified.'
      );
      return;
    }
  ...
  View.prototype[view_common_1.automationTextProperty.setNative] = function(
    value
  ) {
  ...
  View.prototype[
    view_common_1.isUserInteractionEnabledProperty.getDefault
  ] = function() {
    return this.nativeViewProtected.userInteractionEnabled;
  };
  View.prototype[
    view_common_1.isUserInteractionEnabledProperty.setNative
  ] = function(value) {
    this.nativeViewProtected.userInteractionEnabled = value;
  };
  View.prototype[
    style_properties_1.visibilityProperty.getDefault
  ] = function() {
    return this.nativeViewProtected.hidden
      ? style_properties_1.Visibility.COLLAPSE
      : style_properties_1.Visibility.VISIBLE;
  };
  View.prototype[style_properties_1.visibilityProperty.setNative] = function(
    value
  ) {
    switch (value) {
      ...
      default:
        throw new Error(
          'Invalid visibility value: ' +
            ...
            style_properties_1.Visibility.VISIBLE +
            '", "' +
            style_properties_1.Visibility.HIDDEN +
            '", "' +
            style_properties_1.Visibility.COLLAPSE +
            '".'
        );
  ...
  View.prototype[style_properties_1.opacityProperty.setNative] = function(
    value
  ) {
    var nativeView = this.nativeViewProtected;
    var updateSuspended = this._isPresentationLayerUpdateSuspeneded();
  ...
  View.prototype[style_properties_1.rotateProperty.setNative] = function(
    value
  ) {
  ...
  View.prototype[style_properties_1.scaleXProperty.setNative] = function(
    value
  ) {
    this.updateNativeTransform();
  };
  ...
  View.prototype[style_properties_1.scaleYProperty.setNative] = function(
    value
  ) {
    this.updateNativeTransform();
  };
  View.prototype[
    style_properties_1.translateXProperty.getDefault
  ] = function() {
    return 0;
  };
  View.prototype[style_properties_1.translateXProperty.setNative] = function(
    value
  ) {
    this.updateNativeTransform();
  };
  View.prototype[
    style_properties_1.translateYProperty.getDefault
  ] = function() {
    return 0;
  };
  View.prototype[style_properties_1.translateYProperty.setNative] = function(
    value
  ) {
    this.updateNativeTransform();
  };
  ...
  View.prototype[style_properties_1.zIndexProperty.setNative] = function(
    value
  ) {
    this.nativeViewProtected.layer.zPosition = value;
  };
  View.prototype[
    style_properties_1.backgroundInternalProperty.getDefault
  ] = function() {
    return this.nativeViewProtected.backgroundColor;
  };
  View.prototype[
    style_properties_1.backgroundInternalProperty.setNative
  ] = function(value) {
    this._nativeBackgroundState = 'invalid';
  ...
  View.prototype._getCurrentLayoutBounds = function() {
    ...
    if (nativeView && !this.isCollapsed) {
      ...
      return {
        ...
        right: Math.round(
          view_common_1.layout.toDevicePixels(origin_2.x + size.width)
        ),
        bottom: Math.round(
          view_common_1.layout.toDevicePixels(origin_2.y + size.height)
        )
      };
    } else {
  ...
  View.prototype._setupPopoverControllerDelegate = function(
    controller,
    parent
  ) {
    var popoverPresentationController =
      controller.popoverPresentationController;
    this._popoverPresentationDelegate = view_helper_1.ios.UIPopoverPresentationControllerDelegateImp.initWithOwnerAndCallback(
      new WeakRef(this),
  ...
  View.prototype._setupAdaptiveControllerDelegate = function(controller) {
    this._adaptivePresentationDelegate = view_helper_1.ios.UIAdaptivePresentationControllerDelegateImp.initWithOwnerAndCallback(
      new WeakRef(this),
      this._closeModalCallback
    );
...
var CustomLayoutView = (function(_super) {
  ...
  CustomLayoutView.prototype.onMeasure = function(
    widthMeasureSpec,
    heightMeasureSpec
  ) {};
  CustomLayoutView.prototype._addViewToNativeVisualTree = function(
    child,
    atIndex
  ) {
    ...
    if (parentNativeView && childNativeView) {
      if (
        ...
        atIndex >= parentNativeView.subviews.count
      ) {
