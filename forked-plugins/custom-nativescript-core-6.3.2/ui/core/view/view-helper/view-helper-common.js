...
var ViewHelper = (function() {
  function ViewHelper() {}
  ViewHelper.measureChild = function(
    parent,
    child,
    widthMeasureSpec,
    heightMeasureSpec
    ...
    if (child && !child.isCollapsed) {
      var widthSpec = parent
        ? parent._currentWidthMeasureSpec
        : widthMeasureSpec;
      var heightSpec = parent
      ...
      var childWidthMeasureSpec = ViewHelper.getMeasureSpec(
        widthMeasureSpec,
        horizontalMargins,
        child.effectiveWidth,
        style.horizontalAlignment === 'stretch'
      );
      var childHeightMeasureSpec = ViewHelper.getMeasureSpec(
        heightMeasureSpec,
        verticalMargins,
      ...
      if (trace_1.isEnabled()) {
        trace_1.write(
          child.parent +
            ...
            child +
            ' ' +
            utils_1.layout.measureSpecToString(childWidthMeasureSpec) +
            ', ' +
            utils_1.layout.measureSpecToString(childHeightMeasureSpec) +
            '}',
  ...
  ViewHelper.layoutChild = function(
    parent,
    child,
    left,
    top,
  ...
  ViewHelper.resolveSizeAndState = function(
    size,
    specSize,
    specMode,
    childMeasuredState
    ...
    switch (specMode) {
      ...
      case utils_1.layout.AT_MOST:
        if (specSize < size) {
          result =
            Math.ceil(specSize) | utils_1.layout.MEASURED_STATE_TOO_SMALL;
        }
        break;
  ...
  ViewHelper.getMeasureSpec = function(
    parentSpec,
    margins,
    childLength,
    stretched
    ...
    if (childLength >= 0) {
      resultSize =
        parentSpecMode === utils_1.layout.UNSPECIFIED
          ? childLength
      ...
      switch (parentSpecMode) {
        case utils_1.layout.EXACTLY:
          ...
          resultMode = stretched
            ? utils_1.layout.EXACTLY
            : utils_1.layout.AT_MOST;
          break;
        case utils_1.layout.AT_MOST:
          resultSize = Math.max(0, parentLength - margins);
