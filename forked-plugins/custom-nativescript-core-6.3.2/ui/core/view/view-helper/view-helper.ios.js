...
var ios;
(function(ios) {
  ios.traitCollectionColorAppearanceChangedEvent =
  ...
  function layoutView(controller, owner) {
    ...
    view_helper_common_1.ViewHelper.measureChild(
      null,
      owner,
      widthSpec,
      heightSpec
    );
    view_helper_common_1.ViewHelper.layoutChild(
      null,
      owner,
      position.left,
      position.top,
  ...
  function getPositionFromFrame(frame) {
    var left = utils_1.layout.round(
      utils_1.layout.toDevicePixels(frame.origin.x)
    );
    var top = utils_1.layout.round(
      utils_1.layout.toDevicePixels(frame.origin.y)
    );
    var right = utils_1.layout.round(
      utils_1.layout.toDevicePixels(frame.origin.x + frame.size.width)
    );
    var bottom = utils_1.layout.round(
      utils_1.layout.toDevicePixels(frame.origin.y + frame.size.height)
    );
  ...
  function getFrameFromPosition(position, insets) {
    ...
    var top = utils_1.layout.toDeviceIndependentPixels(
      position.top + insets.top
    );
    var width = utils_1.layout.toDeviceIndependentPixels(
      position.right - position.left - insets.left - insets.right
    );
    var height = utils_1.layout.toDeviceIndependentPixels(
    ...
    if (insets.left || insets.top) {
      ...
      if (trace_1.isEnabled()) {
        trace_1.write(
          this +
            ' :shrinkToSafeArea: ' +
            JSON.stringify(ios.getPositionFromFrame(adjustedFrame)),
          trace_1.categories.Layout
        );
      }
  ...
  function expandBeyondSafeArea(view, frame) {
    ...
    if (
      ...
      adjustedPosition.right +=
        fullscreenPosition.right - inWindowPosition.right;
    }
    if (
      inWindowPosition.bottom < fullscreenPosition.bottom &&
      inWindowPosition.bottom >=
        safeAreaPosition.bottom + fullscreenPosition.top
    ) {
      adjustedPosition.bottom +=
        fullscreenPosition.bottom - inWindowPosition.bottom;
    }
    var adjustedFrame = CGRectMake(
      utils_1.layout.toDeviceIndependentPixels(adjustedPosition.left),
      utils_1.layout.toDeviceIndependentPixels(adjustedPosition.top),
      utils_1.layout.toDeviceIndependentPixels(
        adjustedPosition.right - adjustedPosition.left
      ),
      utils_1.layout.toDeviceIndependentPixels(
        adjustedPosition.bottom - adjustedPosition.top
      )
    );
    if (trace_1.isEnabled()) {
      trace_1.write(
  ...
  function getAvailableSpaceFromParent(view, frame) {
    ...
    if (view.viewController) {
      ...
      while (
        parent_1 &&
        !parent_1.viewController &&
        !(parent_1.nativeViewProtected instanceof UIScrollView)
      ) {
  ...
  var UILayoutViewController = (function(_super) {
    ...
    UILayoutViewController.prototype.viewDidLayoutSubviews = function() {
      ...
      if (owner) {
        if (majorVersion >= 11) {
          ...
          if (parent_2) {
            var parentPageInsetsTop =
              parent_2.nativeViewProtected.safeAreaInsets.top;
            var currentInsetsTop = this.view.safeAreaInsets.top;
            var additionalInsetsTop = Math.max(
              parentPageInsetsTop - currentInsetsTop,
              0
            );
            var parentPageInsetsBottom =
              parent_2.nativeViewProtected.safeAreaInsets.bottom;
            var currentInsetsBottom = this.view.safeAreaInsets.bottom;
            var additionalInsetsBottom = Math.max(
    ...
    UILayoutViewController.prototype.traitCollectionDidChange = function(
      ...
      if (majorVersion >= 13) {
        ...
        if (
          ...
          owner.notify({
            eventName: ios.traitCollectionColorAppearanceChangedEvent,
            object: owner
          });
        }
      }
  ...
  var UIAdaptivePresentationControllerDelegateImp = (function(_super) {
    ...
    UIAdaptivePresentationControllerDelegateImp.initWithOwnerAndCallback = function(
      owner,
      whenClosedCallback
    ) {
    ...
    UIAdaptivePresentationControllerDelegateImp.prototype.presentationControllerDidDismiss = function(
    ...
    UIAdaptivePresentationControllerDelegateImp.ObjCProtocols = [
  ...
  var UIPopoverPresentationControllerDelegateImp = (function(_super) {
    ...
    UIPopoverPresentationControllerDelegateImp.initWithOwnerAndCallback = function(
      owner,
      whenClosedCallback
    ) {
      var instance = _super.new.call(this);
    ...
    UIPopoverPresentationControllerDelegateImp.prototype.popoverPresentationControllerDidDismissPopover = function(
    ...
    UIPopoverPresentationControllerDelegateImp.ObjCProtocols = [
