...
var ViewCommon = (function(_super) {
  ...
  ViewCommon.prototype._updateStyleScope = function(
    cssFileName,
    cssString,
    css
  ) {
  ...
  ViewCommon.prototype.setScopeProperty = function(
    scope,
    cssFileName,
    cssString,
    css
  ...
  ViewCommon.prototype._onLivesync = function(context) {
    if (view_base_1.traceEnabled()) {
      view_base_1.traceWrite(
        this + '._onLivesync(' + JSON.stringify(context) + ')',
        view_base_1.traceCategories.Livesync
      );
    }
  ...
  ViewCommon.prototype._handleLivesync = function(context) {
    if (view_base_1.traceEnabled()) {
      view_base_1.traceWrite(
        this + '._handleLivesync(' + JSON.stringify(context) + ')',
        view_base_1.traceCategories.Livesync
      );
    }
    if (viewMatchesModuleContext(this, context, ['style'])) {
      if (view_base_1.traceEnabled()) {
        view_base_1.traceWrite(
          'Change Handled: Changing CSS for ' + this,
          view_base_1.traceCategories.Livesync
        );
      }
      var cssModuleName =
        module_name_sanitizer_1.sanitizeModuleName(context.path) + '.css';
      this.changeCssFile(cssModuleName);
      return true;
    }
    if (
      viewMatchesModuleContext(this, context, ['markup', 'script']) &&
      this.page &&
      this.page.frame
    ) {
      if (view_base_1.traceEnabled()) {
        view_base_1.traceWrite(
          'Change Handled: Changing ' +
            context.type +
            ' for ' +
            this +
            ' inside ' +
  ...
  ViewCommon.prototype._observe = function(type, callback, thisArg) {
    ...
    this._gestureObservers[type].push(
      gestures_1.observe(this, type, callback, thisArg)
    );
  };
        ...
        if (events.length > 0) {
          for (var i = 0; i < events.length; i++) {
            ...
            if (gst && !this._isEvent(arg)) {
              ...
              _super.prototype.addEventListener.call(
                this,
                evt,
                callback,
                thisArg
    ...
    if (args.length === 0) {
      ...
      if (args.length === 2) {
        ...
        if (args[0] instanceof ViewCommon) {
          console.log(
            'showModal(view: ViewBase, context: any, closeCallback: Function, fullscreen?: boolean, animated?: boolean, stretched?: boolean) ' +
              'is deprecated. Use showModal(view: ViewBase, modalOptions: ShowModalOptions) instead.'
          );
        } else {
          console.log(
            'showModal(moduleName: string, context: any, closeCallback: Function, fullscreen?: boolean, animated?: boolean, stretched?: boolean) ' +
      ...
      var view =
        firstArgument instanceof ViewCommon
          ? firstArgument
          : builder_1.Builder.createViewFromEntry({
  ...
  ViewCommon.prototype._showNativeModalView = function(parent, options) {
    ...
    this.cssClasses.add(system_classes_1.MODAL_ROOT_VIEW_CSS_CLASS);
    var modalRootViewCssClasses = system_classes_1.getSystemCssClasses();
    modalRootViewCssClasses.forEach(function(c) {
      return _this.cssClasses.add(c);
    });
    parent._modal = this;
  ...
  ViewCommon.prototype.getMeasuredState = function() {
    return (
      (this._measuredWidth & view_base_1.layout.MEASURED_STATE_MASK) |
      ((this._measuredHeight >>
        view_base_1.layout.MEASURED_HEIGHT_STATE_SHIFT) &
        (view_base_1.layout.MEASURED_STATE_MASK >>
          view_base_1.layout.MEASURED_HEIGHT_STATE_SHIFT))
    );
  };
  ViewCommon.prototype.setMeasuredDimension = function(
    measuredWidth,
    measuredHeight
  ) {
    ...
    if (view_base_1.traceEnabled()) {
      view_base_1.traceWrite(
        this +
          ...
          ', ' +
          measuredHeight,
        view_base_1.traceCategories.Layout
      );
    }
  };
  ...
  ViewCommon.resolveSizeAndState = function(
    size,
    specSize,
    specMode,
    childMeasuredState
  ) {
    return view_helper_1.ViewHelper.resolveSizeAndState(
      size,
      specSize,
      specMode,
      childMeasuredState
  ...
  ViewCommon.combineMeasuredStates = function(curState, newState) {
    return view_helper_1.ViewHelper.combineMeasuredStates(curState, newState);
  };
  ViewCommon.layoutChild = function(
    parent,
    child,
    left,
    top,
    ...
    view_helper_1.ViewHelper.layoutChild(
      parent,
      child,
      left,
      top,
  ...
  ViewCommon.measureChild = function(
    parent,
    child,
    widthMeasureSpec,
    heightMeasureSpec
  ) {
    return view_helper_1.ViewHelper.measureChild(
      parent,
      child,
      widthMeasureSpec,
      heightMeasureSpec
  ...
  ViewCommon.prototype._setCurrentMeasureSpecs = function(
    widthMeasureSpec,
    heightMeasureSpec
  ) {
    var changed =
  ...
  ViewCommon.prototype._setCurrentLayoutBounds = function(
    left,
    top,
    right,
    bottom
    ...
    var sizeChanged =
      this._oldRight - this._oldLeft !== right - left ||
      this._oldBottom - this._oldTop !== bottom - top;
    this._oldLeft = left;
    this._oldTop = top;
    this._oldRight = right;
    this._oldBottom = bottom;
  ...
  ViewCommon.prototype.getActualSize = function() {
    ...
    return {
      width: view_base_1.layout.toDeviceIndependentPixels(
        currentBounds.right - currentBounds.left
      ),
      height: view_base_1.layout.toDeviceIndependentPixels(
  ...
  ViewCommon.prototype._getValue = function() {
    throw new Error(
      'The View._getValue is obsolete. There is a new property system.'
    );
  };
  ViewCommon.prototype._setValue = function() {
    throw new Error(
      'The View._setValue is obsolete. There is a new property system.'
    );
  };
  ViewCommon.prototype._updateEffectiveLayoutValues = function(
    ...
    var availableWidth =
      parentWidthMeasureMode === view_base_1.layout.UNSPECIFIED
        ? -1
        : parentWidthMeasureSize;
    this.effectiveWidth = style_properties_1.PercentLength.toDevicePixels(
      style.width,
      -2,
      availableWidth
    );
    this.effectiveMarginLeft = style_properties_1.PercentLength.toDevicePixels(
      style.marginLeft,
      0,
      availableWidth
    );
    this.effectiveMarginRight = style_properties_1.PercentLength.toDevicePixels(
      style.marginRight,
      0,
      availableWidth
    );
    var availableHeight =
      parentHeightMeasureMode === view_base_1.layout.UNSPECIFIED
        ? -1
        : parentHeightMeasureSize;
    this.effectiveHeight = style_properties_1.PercentLength.toDevicePixels(
      style.height,
      -2,
      availableHeight
    );
    this.effectiveMarginTop = style_properties_1.PercentLength.toDevicePixels(
      style.marginTop,
      0,
      availableHeight
    );
    this.effectiveMarginBottom = style_properties_1.PercentLength.toDevicePixels(
      style.marginBottom,
      0,
      availableHeight
    );
