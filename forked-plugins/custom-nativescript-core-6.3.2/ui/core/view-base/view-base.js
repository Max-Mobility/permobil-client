...
(function(SuspendType) {
  ...
  SuspendType[(SuspendType['IncrementalCountMask'] = -7340033)] =
    'IncrementalCountMask';
})(SuspendType || (SuspendType = {}));
(function(SuspendType) {
  function toString(type) {
    return (
      (type ? 'suspended' : 'resumed') +
      '(' +
      'Incremental: ' +
      (type & SuspendType.IncrementalCountMask) +
      ', ' +
      'Loaded: ' +
      !(type & SuspendType.Loaded) +
      ', ' +
      'NativeView: ' +
      !(type & SuspendType.NativeView) +
      ', ' +
      'UISetup: ' +
      !(type & SuspendType.UISetup) +
      ')'
    );
  }
...
var ViewBase = (function(_super) {
  ...
  ViewBase.prototype._resumeNativeUpdates = function(type) {
    if (type) {
      ...
      if (
        (this._suspendNativeUpdatesCount & SuspendType.IncrementalCountMask) ===
        0
      ) {
  ...
  ViewBase.prototype._addView = function(view, atIndex) {
    if (bindable_1.traceEnabled()) {
      bindable_1.traceWrite(
        this + '._addView(' + view + ', ' + atIndex + ')',
        bindable_1.traceCategories.ViewHierarchy
      );
    }
    if (!view) {
    ...
    if (view.parent) {
      throw new Error(
        'View already has a parent. View: ' + view + ' Parent: ' + view.parent
      );
    }
    view.parent = this;
  ...
  ViewBase.prototype._removeView = function(view) {
    ...
    if (view.parent !== this) {
      throw new Error(
        'View not added to this instance. View: ' +
          view +
          ' CurrentParent: ' +
          view.parent +
          ' ExpectedParent: ' +
  ...
  ViewBase.prototype._goToVisualState = function(state) {
    if (bindable_1.traceEnabled()) {
      bindable_1.traceWrite(
        this + ' going to state: ' + state,
        bindable_1.traceCategories.Style
      );
    }
  ...
  ViewBase.prototype._applyXmlAttribute = function(attribute, value) {
    console.log(
      'ViewBase._applyXmlAttribute(...) is deprecated; set attributes as plain properties instead'
    );
    if (
      attribute === 'style' ||
      attribute === 'rows' ||
...
ViewBase.prototype._suspendNativeUpdatesCount =
  SuspendType.Loaded | SuspendType.NativeView | SuspendType.UISetup;
...
exports.classNameProperty = new properties_1.Property({
  ...
  valueChanged: function(view, oldValue, newValue) {
    ...
    var rootViewsCssClasses = system_classes_1.getSystemCssClasses();
    var shouldAddModalRootViewCssClasses = cssClasses.has(
      system_classes_1.MODAL_ROOT_VIEW_CSS_CLASS
    );
    var shouldAddRootViewCssClasses = cssClasses.has(
      system_classes_1.ROOT_VIEW_CSS_CLASS
    );
    cssClasses.clear();
    if (shouldAddModalRootViewCssClasses) {
      cssClasses.add(system_classes_1.MODAL_ROOT_VIEW_CSS_CLASS);
    } else if (shouldAddRootViewCssClasses) {
      cssClasses.add(system_classes_1.ROOT_VIEW_CSS_CLASS);
    }
    rootViewsCssClasses.forEach(function(c) {
      return cssClasses.add(c);
    });
