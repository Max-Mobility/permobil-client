...
var bindableResources = require('./bindable-resources');
...
function getProperties(property) {
  ...
  result = property
    .replace(binding_builder_1.parentsRegex, 'parentsMatch')
    .replace(/\]/g, '')
    .split(/\.|\[/);
...
function isEventOrGesture(name, view) {
  if (typeof name === 'string') {
    ...
    return (
      (view.constructor && evt in view.constructor) ||
      isGesture(eventOrGestureName.toLowerCase())
    );
  }
  return false;
}
...
var Binding = (function() {
  function Binding(target, options) {
    ...
    this.targetOptions = this.resolveOptions(
      target,
      getProperties(options.targetProperty)
    );
    if (!this.targetOptions) {
      throw new Error(
        'Invalid property: ' + options.targetProperty + ' for target: ' + target
      );
    }
    if (options.twoWay) {
      ...
      if (target_1 instanceof observable_1.Observable) {
        target_1.on(
          this.targetOptions.property + 'Change',
  ...
  Binding.prototype.clearSource = function() {
    ...
    this.propertyChangeListeners.forEach(function(observable, index, map) {
      weak_event_listener_1.removeWeakEventListener(
        observable,
        observable_1.Observable.propertyChangeEvent,
        _this.onSourcePropertyChanged,
        _this
      );
    });
  ...
  Binding.prototype.bind = function(source) {
    var target = this.targetOptions.instance.get();
    if (
      this.sourceIsBindingContext &&
      target instanceof observable_1.Observable &&
      this.targetOptions.property !== 'bindingContext'
    ) {
  ...
  Binding.prototype.resolveObjectsAndProperties = function(source, properties) {
    ...
    for (
      var i = 0, propsArrayLength = properties.length;
      i < propsArrayLength;
      i++
    ) {
      ...
      if (
        property === bc.parentValueKey ||
        property.indexOf(bc.parentsValueKey) === 0
      ) {
        ...
        if (parentView) {
          ...
          targetInstance.off(
            'loaded',
            this.loadedHandlerVisualTreeBinding,
            this
          );
          targetInstance.on(
            'loaded',
  ...
  Binding.prototype.addPropertyChangeListeners = function(
    source,
    sourceProperty,
    parentProperies
  ) {
    var objectsAndProperties = this.resolveObjectsAndProperties(
      source.get(),
      sourceProperty
    );
    ...
    for (var i = 0, length_1 = objectsAndProperties.length; i < length_1; i++) {
      ...
      if (
        !this.propertyChangeListeners.has(prop) &&
        currentObject instanceof observable_1.Observable &&
        currentObject._isViewBase
      ) {
        weak_event_listener_1.addWeakEventListener(
          currentObject,
          propName + 'Change',
  ...
  Binding.prototype.prepareExpressionForUpdate = function() {
    var escapedSourceProperty = utils_1.escapeRegexSymbols(
      this.options.sourceProperty
    );
    var expRegex = new RegExp(escapedSourceProperty, 'g');
    var resultExp = this.options.expression.replace(
      expRegex,
      bc.newPropertyValueKey
    );
    return resultExp;
  };
  Binding.prototype.updateTwoWay = function(value) {
    ...
    if (this.options.expression) {
      ...
      if (this.sourceOptions) {
        ...
        typeof this.options.sourceProperty === 'string' &&
        this.options.sourceProperty.indexOf('.') === -1
      ) {
      ...
      this.prepareContextForExpression(
        changedModel,
        updateExpression,
        undefined
      );
      var expressionValue = this._getExpressionValue(
        updateExpression,
        true,
        changedModel
      );
  ...
  Binding.prototype._getExpressionValue = function(
    expression,
    isBackConvert,
    changedModel
  ) {
    try {
      var exp_1 = polymerExpressions.PolymerExpressions.getExpression(
        expression
      );
      if (exp_1) {
        var context_1 =
          (this.source && this.source.get && this.source.get()) || global;
        var model = {};
        var addedProps = [];
        var resources = bindableResources.get();
        for (var prop in resources) {
          if (
            resources.hasOwnProperty(prop) &&
            !context_1.hasOwnProperty(prop)
          ) {
        ...
        var result = exp_1.getValue(
          model,
          isBackConvert,
          changedModel ? changedModel : model
        );
      ...
      var errorMessage =
        'Run-time error occured in file: ' +
        e.sourceURL +
        ' at line: ' +
        e.line +
        ' and column: ' +
  ...
  Binding.prototype.onSourcePropertyChanged = function(data) {
    ...
    if (changedPropertyIndex > -1) {
      parentProps =
        '$' + sourceProps.slice(0, changedPropertyIndex + 1).join('$');
      while (this.propertyChangeListeners.get(parentProps) !== data.object) {
        changedPropertyIndex +=
          sourceProps
            .slice(changedPropertyIndex + 1)
            .indexOf(data.propertyName) + 1;
        parentProps =
          '$' + sourceProps.slice(0, changedPropertyIndex + 1).join('$');
      }
    }
    ...
    if (
      ...
      if (
        ...
        if (
          !types.isNullOrUndefined(newObject) &&
          typeof newObject === 'object'
        ) {
          this.addPropertyChangeListeners(
            new WeakRef(newObject),
            newProps,
            parentProps
          );
        }
  ...
  Binding.prototype.prepareContextForExpression = function(
    model,
    expression,
    newProps
  ) {
    ...
    if (expression.indexOf(bc.parentValueKey) > -1) {
      parentView = this.getParentView(this.target.get(), bc.parentValueKey)
        .view;
      if (parentView) {
    ...
    if (parentsArray) {
      for (var i = 0; i < parentsArray.length; i++) {
        parentViewAndIndex = this.getParentView(
          this.target.get(),
          parentsArray[i]
        );
        if (parentViewAndIndex.view) {
          model[bc.parentsValueKey] = model[bc.parentsValueKey] || {};
          model[bc.parentsValueKey][parentViewAndIndex.index] =
            parentViewAndIndex.view.bindingContext;
  ...
  Binding.prototype.getSourcePropertyValue = function() {
    if (this.options.expression) {
      var changedModel = {};
      changedModel[bc.bindingValueKey] = this.source
        ? this.source.get()
        : undefined;
      var expressionValue = this._getExpressionValue(
        this.options.expression,
    ...
    if (this.sourceOptions) {
      ...
      if (this.sourceOptions.property === bc.bindingValueKey) {
        ...
        sourceOptionsInstance instanceof observable_1.Observable &&
        this.sourceOptions.property &&
        this.sourceOptions.property !== ''
      ) {
        ...
        trace_1.write(
          "Property: '" +
            this.sourceOptions.property +
            "' is invalid or does not exist. SourceProperty: '" +
            this.options.sourceProperty +
            "'",
          trace_1.categories.Binding,
          trace_1.messageType.error
  ...
  Binding.prototype.updateTarget = function(value) {
    ...
    this.updateOptions(
      this.targetOptions,
      types.isNullOrUndefined(value) ? properties_1.unsetValue : value
    );
  };
  ...
  Binding.prototype.resolveOptions = function(obj, properties) {
    ...
    if (objectsAndProperties.length > 0) {
      var resolvedObj =
        objectsAndProperties[objectsAndProperties.length - 1].instance;
  ...
  Binding.prototype.updateOptions = function(options, value) {
    ...
    try {
      if (
        isEventOrGesture(options.property, optionsInstance) &&
        types.isFunction(value)
      ) {
        optionsInstance.off(
          options.property,
          null,
          optionsInstance.bindingContext
        );
        optionsInstance.on(
          options.property,
          value,
      ...
      trace_1.write(
        'Binding error while setting property ' +
          options.property +
          ' of ' +
          optionsInstance +
          ': ' +
