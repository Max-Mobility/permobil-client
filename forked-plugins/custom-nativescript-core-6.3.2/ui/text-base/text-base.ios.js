...
var TextBase = (function(_super) {
  ...
  TextBase.prototype[
    ...
    var nativeView = this.nativeTextViewProtected;
    nativeView =
      nativeView instanceof UIButton ? nativeView.titleLabel : nativeView;
  ...
  TextBase.prototype[
    ...
    if (!(value instanceof font_1.Font) || !this.formattedText) {
      ...
      nativeView =
        nativeView instanceof UIButton ? nativeView.titleLabel : nativeView;
      var font =
        value instanceof font_1.Font ? value.getUIFont(nativeView.font) : value;
  ...
  TextBase.prototype.setFormattedTextDecorationAndTransform = function() {
    ...
    if (this.letterSpacing !== 0) {
      attrText.addAttributeValueRange(
        NSKernAttributeName,
        this.letterSpacing * this.nativeTextViewProtected.font.pointSize,
  ...
  TextBase.prototype.setTextDecorationAndTransform = function() {
    ...
    switch (style.textDecoration) {
      ...
      default:
        throw new Error(
          'Invalid text decoration value: ' +
            style.textDecoration +
            ". Valid values are: 'none', 'underline', 'line-through', 'underline line-through'."
        );
    }
    if (style.letterSpacing !== 0) {
      dict.set(
        NSKernAttributeName,
        style.letterSpacing * this.nativeTextViewProtected.font.pointSize
      );
    }
  ...
  TextBase.prototype.createNSMutableAttributedString = function(
    formattedString
  ) {
    var mas = NSMutableAttributedString.alloc().init();
    if (formattedString && formattedString.parent) {
      for (
        var i = 0, spanStart = 0, length_1 = formattedString.spans.length;
        i < length_1;
        i++
      ) {
        ...
        var nsAttributedString = this.createMutableStringForSpan(
          span,
          spanText
        );
  ...
  TextBase.prototype.createMutableStringForSpan = function(span, text) {
    ...
    return NSMutableAttributedString.alloc().initWithStringAttributes(
      text,
      attrDict
    );
  };
...
function NSStringFromNSAttributedString(source) {
  return NSString.stringWithString(
    (source instanceof NSAttributedString && source.string) || source
  );
}
