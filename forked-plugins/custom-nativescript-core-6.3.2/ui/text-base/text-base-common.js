...
var formatted_string_1 = require('./formatted-string');
...
var TextBaseCommon = (function(_super) {
  ...
  TextBaseCommon.prototype._addChildFromBuilder = function(name, value) {
    if (name === CHILD_SPAN) {
      ...
      name === CHILD_FORMATTED_TEXT ||
      name === CHILD_FORMATTED_STRING
    ) {
...
function onFormattedTextPropertyChanged(textBase, oldValue, newValue) {
  if (oldValue) {
    oldValue.off(
      view_1.Observable.propertyChangeEvent,
      textBase._onFormattedTextContentsChanged,
      textBase
    );
...
var textAlignmentConverter = view_1.makeParser(
  view_1.makeValidator('initial', 'left', 'center', 'right')
);
...
var textTransformConverter = view_1.makeParser(
  view_1.makeValidator(
    'initial',
    'none',
    'capitalize',
    'uppercase',
...
var whiteSpaceConverter = view_1.makeParser(
  view_1.makeValidator('initial', 'normal', 'nowrap')
);
...
var textDecorationConverter = view_1.makeParser(
  view_1.makeValidator(
    'none',
    'underline',
    'line-through',
