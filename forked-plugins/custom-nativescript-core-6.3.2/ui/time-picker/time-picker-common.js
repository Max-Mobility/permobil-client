...
function getValidTime(picker, hour, minute) {
  if (picker.minuteInterval > 1) {
    ...
    minute =
      minuteFloor + (minute === minuteFloor + 1 ? picker.minuteInterval : 0);
    if (minute === 60) {
      hour++;
      minute = 0;
    }
...
function isMinuteIntervalValid(value) {
  return (
    typeof value === 'number' && value >= 1 && value <= 30 && 60 % value === 0
  );
}
...
function isGreaterThanMinTime(picker, hour, minute) {
  ...
  return (
    getMinutes(hour !== undefined ? hour : picker.hour) +
      (minute !== undefined ? minute : picker.minute) >=
    getMinutes(picker.minHour) + picker.minMinute
  );
}
function isLessThanMaxTime(picker, hour, minute) {
  ...
  return (
    getMinutes(isDefined(hour) ? hour : picker.hour) +
      (isDefined(minute) ? minute : picker.minute) <=
    getMinutes(picker.maxHour) + picker.maxMinute
  );
}
...
function getMinMaxTimeErrorMessage(picker) {
  return (
    'Min time: (' +
    toString(picker.minHour) +
    ':' +
    toString(picker.minMinute) +
    '), max time: (' +
    toString(picker.maxHour) +
    ':' +
    toString(picker.maxMinute) +
    ')'
  );
}
function getErrorMessage(picker, propertyName, newValue) {
  return (
    ...
    toString(newValue) +
    ':' +
    toString(picker.minute) +
    ') is not valid. ' +
    getMinMaxTimeErrorMessage(picker) +
    '.'
  );
}
