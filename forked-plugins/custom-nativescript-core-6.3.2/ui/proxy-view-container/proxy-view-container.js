...
var ProxyViewContainer = (function(_super) {
  ...
  ProxyViewContainer.prototype._addViewToNativeVisualTree = function(
    child,
    atIndex
  ) {
    var _this = this;
    if (layout_base_1.traceEnabled()) {
      layout_base_1.traceWrite(
        'ProxyViewContainer._addViewToNativeVisualTree for a child ' +
          child +
          ' ViewContainer.parent: ' +
          this.parent,
        layout_base_1.traceCategories.ViewHierarchy
      );
    }
    ...
    if (parent instanceof layout_base_1.View) {
      ...
      if (parent instanceof layout_base_1.LayoutBase) {
        baseIndex = parent._childIndexToNativeChildIndex(
      ...
      if (layout_base_1.traceEnabled()) {
        layout_base_1.traceWrite(
          'ProxyViewContainer._addViewToNativeVisualTree at: ' +
            atIndex +
            ' base: ' +
            baseIndex +
            ' additional: ' +
  ...
  ProxyViewContainer.prototype._removeViewFromNativeVisualTree = function(
    child
  ) {
    if (layout_base_1.traceEnabled()) {
      layout_base_1.traceWrite(
        'ProxyViewContainer._removeViewFromNativeVisualTree for a child ' +
          child +
          ' ViewContainer.parent: ' +
          this.parent,
        layout_base_1.traceCategories.ViewHierarchy
  ...
  ProxyViewContainer.prototype._changedLayoutProperty = function(
    propName,
    value
  ) {
    var _this = this;
    ...
    if (numChildren > 1) {
      layout_base_1.traceWrite(
        "ProxyViewContainer._changeLayoutProperty - you're setting '" +
          propName +
          "' for " +
          this +
          ' with more than one child. Probably this is not what you want, consider wrapping it in a StackLayout ',
        layout_base_1.traceCategories.ViewHierarchy,
  ...
  ProxyViewContainer.prototype._applyLayoutPropertyToChild = function(
    child,
    propName,
    value
  ) {
    ...
    if (proxyPropName in child) {
      if (child[propName] !== child[proxyPropName]) {
        if (layout_base_1.traceEnabled()) {
          layout_base_1.traceWrite(
            'ProxyViewContainer._applyLayoutPropertyToChild child ' +
              child +
              ' has its own value [' +
              child[propName] +
              '] for [' +
...
for (
  var _i = 0, layoutProperties_1 = layoutProperties;
  _i < layoutProperties_1.length;
  _i++
) {
