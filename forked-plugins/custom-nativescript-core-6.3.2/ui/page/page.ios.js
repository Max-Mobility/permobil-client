...
function isBackNavigationTo(page, entry) {
  ...
  var isReplace =
    navigationContext.navigationType === frame_common_1.NavigationType.replace;
  if (isReplace) {
    return false;
  }
  ...
  for (var i = 0; i < navigationQueue.length; i++) {
    if (navigationQueue[i].entry === entry) {
      return (
        navigationQueue[i].navigationType === frame_common_1.NavigationType.back
      );
    }
  }
  return false;
...
function isBackNavigationFrom(controller, page) {
  ...
  if (
    ...
    controller.navigationController.viewControllers.containsObject(controller)
  ) {
    return false;
  }
  return true;
...
var UIViewControllerImpl = (function(_super) {
  ...
  UIViewControllerImpl.prototype.viewWillAppear = function(animated) {
    ...
    var frame = this.navigationController
      ? this.navigationController.owner
      : null;
    var newEntry = this[ENTRY];
    if (
      !owner._presentedViewController &&
      newEntry &&
      (!frame || frame.currentPage !== owner)
    ) {
      var isBack = isBackNavigationTo(owner, newEntry);
      owner.onNavigatingTo(
        newEntry.entry.context,
        isBack,
        newEntry.entry.bindingContext
      );
    }
    if (frame) {
      if (!owner.parent) {
    ...
    if (!owner._presentedViewController && frame) {
      ...
      if (isReplace) {
        ...
        if (controller) {
          ...
          if (animated_1) {
            controller[TRANSITION] = frame._getNavigationTransition(
              newEntry.entry
            );
          } else {
      ...
      if (frame.canGoBack()) {
        ...
        navigationController.interactivePopGestureRecognizer.enabled =
          owner.enableSwipeBackNavigation;
      } else {
      ...
      if (majorVersion >= 11) {
        ...
        if (frameParent) {
          var parentPageInsetsTop =
            frameParent.nativeViewProtected.safeAreaInsets.top;
          ...
          var parentPageInsetsBottom =
            frameParent.nativeViewProtected.safeAreaInsets.bottom;
  ...
  UIViewControllerImpl.prototype.traitCollectionDidChange = function(
...
var Page = (function(_super) {
  ...
  function Page() {
    ...
    controller.view.backgroundColor = _this._backgroundColor;
    return _this;
  }
  ...
  Page.prototype.onMeasure = function(widthMeasureSpec, heightMeasureSpec) {
    ...
    if (this.frame && this.frame._getNavBarVisible(this)) {
      var _a = this.actionBar._getActualSize,
        width_1 = _a.width,
        height_1 = _a.height;
      var widthSpec = page_common_1.layout.makeMeasureSpec(
        width_1,
        page_common_1.layout.EXACTLY
      );
      var heightSpec = page_common_1.layout.makeMeasureSpec(
        height_1,
        page_common_1.layout.EXACTLY
      );
      page_common_1.View.measureChild(
        this,
        this.actionBar,
        widthSpec,
        heightSpec
    ...
    var result = page_common_1.View.measureChild(
      this,
      this.layoutView,
      widthMeasureSpec,
      heightMeasureSpec
    ...
    var measureHeight = Math.max(
      result.measuredHeight,
      this.effectiveMinHeight
    );
    var widthAndState = page_common_1.View.resolveSizeAndState(
      measureWidth,
      width,
      widthMode,
      0
    );
    var heightAndState = page_common_1.View.resolveSizeAndState(
      measureHeight,
      height,
      heightMode,
      0
  ...
  Page.prototype.onLayout = function(left, top, right, bottom) {
    ...
    page_common_1.View.layoutChild(
      this,
      this.actionBar,
      0,
      0,
    ...
    if (majorVersion <= 10) {
      insets.top = page_common_1.layout.round(
        page_common_1.layout.toDevicePixels(
          this.viewController.view.safeAreaLayoutGuide.layoutFrame.origin.y
        )
      );
    }
  ...
  Page.prototype._addViewToNativeVisualTree = function(child, atIndex) {
    ...
    var viewController =
      child.ios instanceof UIViewController ? child.ios : child.viewController;
    if (viewController) {
    ...
    if (nativeParent && nativeChild) {
      if (
        ...
        atIndex >= nativeParent.subviews.count
      ) {
  ...
  Page.prototype[page_common_1.actionBarHiddenProperty.setNative] = function(
    value
  ) {
  ...
  Page.prototype[page_common_1.statusBarStyleProperty.setNative] = function(
    value
  ) {
    var frame = this.frame;
    if (frame) {
