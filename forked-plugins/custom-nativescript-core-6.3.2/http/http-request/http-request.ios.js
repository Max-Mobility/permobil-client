...
(function(HttpResponseEncoding) {
  ...
  (HttpResponseEncoding =
    exports.HttpResponseEncoding || (exports.HttpResponseEncoding = {}))
);
...
var USER_AGENT =
  ...
  '; CPU OS ' +
  osVersion.replace('.', '_') +
  ' like Mac OS X) AppleWebKit/536.26 (KHTML, like Gecko) Version/' +
  osVersion +
...
var NSURLSessionTaskDelegateImpl = (function(_super) {
  ...
  NSURLSessionTaskDelegateImpl.prototype.URLSessionTaskWillPerformHTTPRedirectionNewRequestCompletionHandler = function(
    session,
    task,
    response,
    request,
...
function ensureDefaultSession() {
  if (!defaultSession) {
    defaultSession = NSURLSession.sessionWithConfigurationDelegateDelegateQueue(
      sessionConfig,
      null,
      queue
    );
...
function ensureSessionNotFollowingRedirects() {
  if (!sessionNotFollowingRedirects) {
    sessionNotFollowingRedirects = NSURLSession.sessionWithConfigurationDelegateDelegateQueue(
      sessionConfig,
      sessionTaskDelegateInstance,
      queue
    );
  }
}
...
function request(options) {
  return new Promise(function(resolve, reject) {
    ...
    try {
      ...
      var session = void 0;
      if (
        types.isBoolean(options.dontFollowRedirects) &&
        options.dontFollowRedirects
      ) {
      ...
      var dataTask = session.dataTaskWithRequestCompletionHandler(
        urlRequest,
        function(data, response, error) {
          if (error) {
            ...
            if (response && response.allHeaderFields) {
              ...
              headerFields.enumerateKeysAndObjectsUsingBlock(function(
                key,
                value,
                stop
              ) {
            ...
            resolve({
              content: {
                ...
                toImage: function() {
                  ensureImageSource();
                  return new Promise(function(resolve, reject) {
                    UIImage.tns_decodeImageWithDataCompletion(data, function(
                      image
                    ) {
                      if (image) {
                ...
                toFile: function(destinationFilePath) {
                  ...
                  if (!destinationFilePath) {
                    destinationFilePath = http_request_common_1.getFilenameFromUrl(
                      options.url
                    );
                  }
