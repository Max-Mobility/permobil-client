...
var ImageAsset = (function(_super) {
  ...
  ImageAsset.prototype.getImageAsync = function(callback) {
    ...
    var srcWidth = this.nativeImage
      ? this.nativeImage.size.width
      : this.ios.pixelWidth;
    var srcHeight = this.nativeImage
      ? this.nativeImage.size.height
      : this.ios.pixelHeight;
    var requestedSize = image_asset_common_1.getRequestedImageSize(
    ...
    PHImageManager.defaultManager().requestImageForAssetTargetSizeContentModeOptionsResultHandler(
      ...
      0,
      imageRequestOptions,
      function(image, imageResultInfo) {
        if (image) {
