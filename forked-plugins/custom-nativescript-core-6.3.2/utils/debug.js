...
function ensureAppRootPath() {
  if (!applicationRootPath) {
    ...
    applicationRootPath = applicationRootPath.substr(
      0,
      applicationRootPath.length - 'app/'.length
    );
  }
}
var Source = (function() {
  function Source(uri, line, column) {
    ensureAppRootPath();
    if (
      uri.length > applicationRootPath.length &&
      uri.substr(0, applicationRootPath.length) === applicationRootPath
    ) {
...
var ScopeError = (function(_super) {
  ...
  function ScopeError(inner, message) {
    ...
    _this.stack = platform_1.isAndroid
      ? 'Error: ' +
        _this.message +
        '\n' +
