...
function openUrl(location) {
  ...
  try {
    var intent = new android.content.Intent(
      ...
      android.net.Uri.parse(location.trim())
    );
...
function openFile(filePath) {
  ...
  try {
    ...
    var chooserIntent = android.content.Intent.createChooser(
      intent,
      'Open File...'
    );
    intent.addFlags(android.content.Intent.FLAG_ACTIVITY_NEW_TASK);
    ...
    if (sdkVersion && sdkVersion < MIN_URI_SHARE_RESTRICTED_APK_VERSION) {
      trace_1.write(
        'detected sdk version ' +
          sdkVersion +
          ' (< ' +
          MIN_URI_SHARE_RESTRICTED_APK_VERSION +
          '), using simple openFile',
        trace_1.categories.Debug
      );
    ...
    trace_1.write(
      'fully-qualified provider name [' + providerName + ']',
      trace_1.categories.Debug
    );
