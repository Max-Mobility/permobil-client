...
function makeMeasureSpec(size, mode) {
  return (
    (Math.round(Math.max(0, size)) & ~layout_helper_common_1.MODE_MASK) |
...
function measureNativeView(nativeView, width, widthMode, height, heightMode) {
  ...
  nativeSize.width = layout_helper_common_1.round(
    toDevicePixels(nativeSize.width)
  );
  nativeSize.height = layout_helper_common_1.round(
    toDevicePixels(nativeSize.height)
  );
  return nativeSize;
}
