...
function makeMeasureSpec(size, mode) {
  if (sdkVersion === undefined) {
    sdkVersion = native_helper_1.ad.getApplicationContext().getApplicationInfo()
      .targetSdkVersion;
    useOldMeasureSpec =
      sdkVersion <= android.os.Build.VERSION_CODES.JELLY_BEAN_MR1;
  }
  if (useOldMeasureSpec) {
    return size + mode;
  }
  return (
    (size & ~layout_helper_common_1.MODE_MASK) |
    (mode & layout_helper_common_1.MODE_MASK)
  );
}
...
function measureNativeView(nativeView, width, widthMode, height, heightMode) {
  var view = nativeView;
  view.measure(
    makeMeasureSpec(width, widthMode),
    makeMeasureSpec(height, heightMode)
  );
  return {
