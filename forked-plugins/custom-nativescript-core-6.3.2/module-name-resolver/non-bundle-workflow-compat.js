...
function register(name, loader) {
  if (trace_1.isEnabled()) {
    trace_1.write(
      ...
      trace_1.categories.ModuleNameResolver
    );
  }
...
function processFile(file) {
  var filePathRelativeToApp = file.path.substr(
  ...
  switch (file.extension.toLocaleLowerCase()) {
    case '.js':
      var noExtPath = filePathRelativeToApp.substr(
        0,
...
function registerModulesFromFileSystem(moduleName) {
  ...
  var parentName = moduleName.substr(
    0,
    moduleName.lastIndexOf(fs.path.separator)
  );
  var parentFolderPath = fs.path.join(
  ...
  var tnsModulesPath = fs.path.join(
    fs.knownFolders.currentApp().path,
    'tns_modules',
    moduleName
  );
  ...
  if (parentName) {
    var tnsParentFolderPath = fs.path.join(
      fs.knownFolders.currentApp().path,
      'tns_modules',
      parentName
    );
