...
var FileNameResolver = (function() {
  ...
  FileNameResolver.prototype.resolveFileNameImpl = function(path, ext) {
    ...
    result = qualifier_matcher_1.findMatch(
      path,
      ext,
      candidates,
      this._context
  ...
  FileNameResolver.prototype.getFileCandidatesFromFolder = function(path, ext) {
    ...
    var folderPath = path.substring(
      0,
      path.lastIndexOf(file_system_1.path.separator) + 1
    );
    if (file_system_1.Folder.exists(folderPath)) {
      ...
      if (trace.isEnabled()) {
        trace.write(
          'Could not find folder ' + folderPath + ' when loading ' + path + ext,
