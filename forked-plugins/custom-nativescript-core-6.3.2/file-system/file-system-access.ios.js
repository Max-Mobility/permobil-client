...
var FileSystemAccess = (function() {
    ...
    try {
      ...
      if (!exists) {
        try {
          ...
          if (onError) {
            onError(
              new Error("Failed to create folder at path '" + path + "': " + ex)
            );
          }
          return undefined;
        }
  ...
  FileSystemAccess.prototype.rename = function(path, newPath, onError) {
    ...
    try {
      ...
      if (onError) {
        onError(
          new Error(
            "Failed to rename '" + path + "' to '" + newPath + "': " + ex
          )
        );
      }
    }
  ...
  FileSystemAccess.prototype.getLogicalRootPath = function() {
    ...
    var resolvedPath = NSString.stringWithString(mainBundlePath)
  ...
  FileSystemAccess.prototype.readTextAsync = function(path, encoding) {
    ...
    return new Promise(function(resolve, reject) {
      try {
        NSString.stringWithContentsOfFileEncodingCompletion(
          path,
          actualEncoding,
  ...
  FileSystemAccess.prototype.writeTextAsync = function(
    path,
    content,
    encoding
  ) {
    ...
    return new Promise(function(resolve, reject) {
      try {
        nsString.writeToFileAtomicallyEncodingCompletion(
          path,
          true,
          actualEncoding,
          function(error) {
  ...
  FileSystemAccess.prototype.writeTextSync = function(
    path,
    content,
    onError,
    encoding
  ...
  FileSystemAccess.prototype.deleteEntity = function(path, onError) {
    ...
    try {
      ...
      if (onError) {
        onError(
          new Error("Failed to delete file at path '" + path + "': " + ex)
        );
      }
    }
  };
  FileSystemAccess.prototype.enumEntities = function(path, callback, onError) {
    try {
      ...
      try {
        ...
        if (onError) {
          onError(
            new Error("Failed to enum files for folder '" + path + "': " + ex)
          );
        }
        return;
      }
