...
function getApplicationContext() {
  if (!applicationContext) {
    applicationContext = application_1
...
var FileSystemAccess = (function() {
  ...
  FileSystemAccess.prototype.rename = function(path, newPath, onError) {
    ...
    if (!javaFile.renameTo(newFile)) {
      if (onError) {
        onError(
          new Error("Failed to rename file '" + path + "' to '" + newPath + "'")
        );
      }
    }
  };
  ...
  FileSystemAccess.prototype.ensureFile = function(
    javaFile,
    isFolder,
    onError
  ) {
    try {
      if (!javaFile.exists()) {
        ...
        if (!created) {
          if (onError) {
            onError(
              'Failed to create new java File for path ' +
