...
function uptime() {
  return global.android
    ? org.nativescript.Process.getUpTime()
    : global.__tns_uptime();
}
...
function print(name) {
  ...
  console.log(
    '---- [' + name + '] STOP total: ' + info.totalTime + ' count:' + info.count
  );
  return info;
}
exports.print = print;
...
try {
  ...
  try {
    console.log(
      'Profiling startup failed to figure out defaults from package.json, error: ' +
        e1
    );
  } catch (e2) {}
}
...
function profile(nameFnOrTarget, fnOrKey, descriptor) {
  if (
    typeof nameFnOrTarget === 'object' &&
    (typeof fnOrKey === 'string' || typeof fnOrKey === 'symbol')
  ) {
    ...
    typeof nameFnOrTarget === 'function' &&
    (typeof fnOrKey === 'string' || typeof fnOrKey === 'symbol')
  ) {
    if (!profileFunctionFactory) {
      return;
    ...
    typeof nameFnOrTarget === 'string' &&
    typeof fnOrKey === 'function'
  ) {
    if (!profileFunctionFactory) {
      return fnOrKey;
    }
    ...
    if (info) {
      if (info.runCount) {
        console.log(
          '---- timer with name [' +
            name +
            "] is currently running and won't be reset"
        );
      } else {
...
function trace(message, start, end) {
  log(
    'Timeline: Modules: ' + message + '  (' + start + 'ms. - ' + end + 'ms.)'
  );
}
exports.trace = trace;
