...
var VirtualArray = (function(_super) {
  ...
  VirtualArray.prototype.getItem = function(index) {
    ...
    if (item === undefined) {
      if (
        index >= 0 &&
        index < this.length &&
        this._requestedIndexes.indexOf(index) < 0 &&
        this._loadedIndexes.indexOf(index) < 0
      ) {
  ...
  VirtualArray.prototype.load = function(index, items) {
    for (var i = 0; i < items.length; i++) {
      ...
      this._requestedIndexes.splice(
        this._requestedIndexes.indexOf(itemIndex),
        1
      );
    ...
    if (this._requestedIndexes.length > 0) {
      for (var i = 0; i < this.loadSize - items.length; i++) {
        this._requestedIndexes.splice(
          this._requestedIndexes.indexOf(index + i),
          1
        );
      }
    }
  ...
  VirtualArray.prototype.requestItems = function(index) {
    ...
    var pageIndex =
      this._loadSize > 0
        ? this._loadSize * Math.floor(index / this._loadSize)
        : index;
    var count = 0;
    var start = -1;
