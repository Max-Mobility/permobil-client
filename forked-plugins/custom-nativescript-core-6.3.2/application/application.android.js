...
var AndroidApplication = (function(_super) {
  ...
  AndroidApplication.prototype.init = function(nativeApp) {
    ...
    this.nativeApp.registerActivityLifecycleCallbacks(
      this.callbacks.lifecycleCallbacks
    );
    this.nativeApp.registerComponentCallbacks(
      this.callbacks.componentCallbacks
    );
  ...
  AndroidApplication.prototype.registerBroadcastReceiver = function(
    intentFilter,
    onReceiveCallback
  ) {
  ...
  AndroidApplication.prototype.unregisterBroadcastReceiver = function(
    intentFilter
  ) {
...
function getNativeApplication() {
  ...
  if (!nativeApp) {
    ...
    if (!nativeApp) {
      throw new Error(
        "Failed to retrieve native Android Application object. If you have a custom android.app.Application type implemented make sure that you've called the '<application-module>.android.init' method."
      );
    }
  }
  return nativeApp;
...
function initLifecycleCallbacks() {
  var setThemeOnLaunch = profiling_1.profile('setThemeOnLaunch', function(
    activity
  ) {
    ...
    if (activityInfo.metaData) {
      var setThemeOnLaunch_1 = activityInfo.metaData.getInt(
        'SET_THEME_ON_LAUNCH',
        -1
      );
  ...
  var notifyActivityCreated = profiling_1.profile(
  ...
  var subscribeForGlobalLayout = profiling_1.profile(
    'subscribeForGlobalLayout',
    function(activity) {
      var rootView = activity
        .getWindow()
        .getDecorView()
        .getRootView();
  ...
  var lifecycleCallbacks = new android.app.Application.ActivityLifecycleCallbacks(
    {
      onActivityCreated: profiling_1.profile('onActivityCreated', function(
        activity,
        savedInstanceState
      ) {
        ...
        if (
          application_common_1.hasListeners(application_common_1.displayedEvent)
        ) {
          subscribeForGlobalLayout(activity);
        }
      }),
      onActivityDestroyed: profiling_1.profile('onActivityDestroyed', function(
        activity
      ) {
      ...
      onActivityPaused: profiling_1.profile('onActivityPaused', function(
        activity
      ) {
      ...
      onActivityResumed: profiling_1.profile('onActivityResumed', function(
        activity
      ) {
      ...
      onActivitySaveInstanceState: profiling_1.profile(
      ...
      onActivityStarted: profiling_1.profile('onActivityStarted', function(
        activity
      ) {
        androidApp.notify({
      ...
      onActivityStopped: profiling_1.profile('onActivityStopped', function(
        activity
      ) {
        androidApp.notify({
...
function initComponentCallbacks() {
  var componentCallbacks = new android.content.ComponentCallbacks2({
    ...
    onConfigurationChanged: profiling_1.profile(
      ...
      function(newConfiguration) {
        ...
        if (androidApp.systemAppearance !== newSystemAppearance) {
          ...
          application_common_1.systemAppearanceChanged(
            rootView,
            newSystemAppearance
          );
          application_common_1.notify({
