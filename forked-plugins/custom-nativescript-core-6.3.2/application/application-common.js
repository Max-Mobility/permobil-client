...
function livesync(rootView, context) {
  ...
  if (context && context.path) {
    ...
    var appStylesFileName_1 = appStylesFullFileName.substring(
      0,
...
function increaseStyleScopeApplicationCssSelectorVersion(rootView) {
  var styleScope =
    ...
    (rootView.currentPage && rootView.currentPage._styleScope);
  if (styleScope) {
...
function applyCssClass(rootView, cssClasses, newCssClass) {
  if (!rootView.cssClasses.has(newCssClass)) {
    cssClasses.forEach(function(cssClass) {
      return removeCssClass(rootView, cssClass);
    });
...
function orientationChanged(rootView, newOrientation) {
  ...
  var newOrientationCssClass =
    '' + system_classes_1.CLASS_PREFIX + newOrientation;
...
function systemAppearanceChanged(rootView, newSystemAppearance) {
  ...
  var newSystemAppearanceCssClass =
    '' + system_classes_1.CLASS_PREFIX + newSystemAppearance;
  applyCssClass(
    rootView,
