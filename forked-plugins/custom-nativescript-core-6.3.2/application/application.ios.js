...
var iOSApplication = (function() {
  ...
  Object.defineProperty(iOSApplication.prototype, 'systemAppearance', {
    get: function() {
      ...
      if (!this._systemAppearance) {
        var userInterfaceStyle = this.rootController.traitCollection
          .userInterfaceStyle;
  ...
  iOSApplication.prototype.addNotificationObserver = function(
    notificationName,
    onReceiveCallback
  ) {
  ...
  iOSApplication.prototype.removeNotificationObserver = function(
    observer,
    notificationName
  ) {
  ...
  iOSApplication.prototype.didFinishLaunchingWithOptions = function(
    notification
  ) {
    if (!displayedOnce) {
  ...
  iOSApplication.prototype.didChangeStatusBarOrientation = function(
    notification
  ) {
    var statusBarOrientation =
      UIApplication.sharedApplication.statusBarOrientation;
  ...
  if (!iosApp.nativeApp) {
    UIApplicationMain(
      ...
      iosApp && iosApp.delegate
        ? NSStringFromClass(iosApp.delegate)
    ...
    if (rootView) {
      var window_1 =
        ...
        (iosApp.nativeApp.windows.count > 0 && iosApp.nativeApp.windows[0]);
      if (window_1) {
          ...
          rootView.on(
            ...
            function() {
              var userInterfaceStyle =
                controller_1.traitCollection.userInterfaceStyle;
              var newSystemAppearance = getSystemAppearanceValue(
                userInterfaceStyle
              );
              if (_this._systemAppearance !== newSystemAppearance) {
                _this._systemAppearance = newSystemAppearance;
...
function getViewController(rootView) {
  ...
  if (!(viewController instanceof UIViewController)) {
    viewController = view_helper_1.ios.UILayoutViewController.initWithOwner(
...
function setRootViewsCssClasses(rootView) {
  ...
  system_classes_1.pushToSystemCssClasses(
    '' + system_classes_1.CLASS_PREFIX + IOS_PLATFORM
  );
  system_classes_1.pushToSystemCssClasses(
    '' + system_classes_1.CLASS_PREFIX + deviceType
  );
  system_classes_1.pushToSystemCssClasses(
    '' + system_classes_1.CLASS_PREFIX + iosApp.orientation
  );
  ...
  rootViewCssClasses.forEach(function(c) {
    return rootView.cssClasses.add(c);
  });
}
function setRootViewsSystemAppearanceCssClass(rootView) {
  if (majorVersion >= 13) {
    var systemAppearanceCssClass =
      '' + system_classes_1.CLASS_PREFIX + iosApp.systemAppearance;
