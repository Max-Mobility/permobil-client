...
var ImageSource = (function() {
  ...
  ImageSource.fromResourceSync = function(name) {
    var nativeSource =
      UIImage.tns_safeImageNamed(name) ||
      UIImage.tns_safeImageNamed(name + '.jpg');
      ...
      try {
        UIImage.tns_safeDecodeImageNamedCompletion(name, function(image) {
          if (image) {
            ...
            UIImage.tns_safeDecodeImageNamedCompletion(name + '.jpg', function(
              image
            ) {
              resolve(new ImageSource(image));
            });
  ...
  ImageSource.fromFile = function(path) {
    return new Promise(function(resolve, reject) {
      try {
        UIImage.tns_decodeImageWidthContentsOfFileCompletion(
          getFileName(path),
          function(uiImage) {
  ...
  ImageSource.fromFileOrResourceSync = function(path) {
    ...
    if (path.indexOf(utils_1.RESOURCE_PREFIX) === 0) {
      return ImageSource.fromResourceSync(
        path.substr(utils_1.RESOURCE_PREFIX.length)
      );
    }
  ...
  ImageSource.fromFontIconCodeSync = function(source, font, color) {
    ...
    var attributes =
      ((_a = {}),
      (_a[NSFontAttributeName] = font.getUIFont(
        UIFont.systemFontOfSize(scaledFontSize)
      )),
      _a);
    if (color) {
      attributes[NSForegroundColorAttributeName] = color.ios;
    }
    var attributedString = NSAttributedString.alloc().initWithStringAttributes(
      source,
      attributes
    );
