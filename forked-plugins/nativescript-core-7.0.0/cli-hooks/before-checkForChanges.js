...
module.exports = function ($staticConfig, hookArgs) {
  ...
  if (
    platfrom.toLowerCase() === 'ios' &&
    !satisfiesRequriredVersion(cliVersion, 6, 2)
  ) {
    throw new Error(
      `Building @nativescript/core for iOS requires NativeScript CLI with version at least 6.2.0. Please upgrade your NativeScript CLI version (npm i -g nativescript).`
    );
  }
  ...
  if (projectData.projectType === 'Angular') {
    ...
    if (webpackMinVer && !satisfiesRequriredVersion(webpackMinVer, 1, 3)) {
      throw new Error(
        `Building @nativescript/core for Angular requires ${webpackPackageName} with version at least 1.3.0. Please upgrade: npm i ${webpackPackageName} --save-dev.`
      );
    }
  }
};
 ...
 */
function satisfiesRequriredVersion(
  actualVersion,
  requiredMajor,
  requiredMinor
) {
  ...
  if (
    actualVersion.major === requiredMajor &&
    actualVersion.minor < requiredMinor
  ) {
    return false;
  }
...
function getMinWebpackVersion(projectData) {
  ...
  const webpackVer =
    dependencies[webpackPackageName] || devDependencies[webpackPackageName];
