...
const setDidAskOnce = (permission: string) =>
  Promise.resolve().then(() =>
    applicationSettings.setBoolean(STORAGE_KEY + permission, true)
  );

const getDidAskOnce = (permission: string) =>
  Promise.resolve(!!applicationSettings.getBoolean(STORAGE_KEY + permission));
...
namespace PermissionsAndroid {
  ...
  export function requestMultiple(
    permissions: string[]
  ): Promise<{ [permission: string]: Status }> {
      ...
      application.android.on(
        ...
        (args: application.AndroidActivityRequestPermissionsEventData) => {
          if (args.requestCode === requestCode) {
            ...
            console.log(
              'did requestMultiplePermissions',
              permissionsToCheck,
              results,
              results.length
            );
...
export function check(permission: string, options?: CheckOptions) {
  if (!permissionTypes[permission]) {
    console.warn(
      `nativescript-perms: ${permission} is not a valid permission type on Android`
    );
      ...
      return getDidAskOnce(permission).then(didAsk => {
        if (didAsk) {
          return shouldShowRequestPermissionRationale(
            ...
            shouldShow => (shouldShow ? 'denied' : 'restricted') as Status
          );
        }
...
export function request(permission: string, options?: RequestOptions) {
  ...
  const rationale =
    typeof options === 'string' ? undefined : options && options.rationale;
