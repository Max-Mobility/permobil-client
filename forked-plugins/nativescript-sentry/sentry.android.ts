...
export class Sentry {
  ...
  public static captureMessage(message: string, options?: MessageOptions) {
    ...
    if (options && options.extra) {
      Object.keys(options.extra).forEach(key => {
        ...
        const nativeDataValue = Sentry._convertDataTypeToJavaObject(
          options.extra[key]
        );
  ...
  public static captureException(exception: Error, options?: ExceptionOptions) {
    ...
    const eventBuilder = new io.sentry.event.EventBuilder()
      ...
      .withSentryInterface(
        new io.sentry.event.interfaces.ExceptionInterface(ex)
      );
    ...
    switch (typeof value) {
      case 'object':
        ...
        Object.keys(value).forEach(itemKey => {
          nativeObject.put(
            itemKey,
            this._convertDataTypeToJavaObject(value[itemKey])
          );
        });
