...
export class Sentry {
  ...
  public static captureMessage(message: string, options?: MessageOptions) {
    ...
    const event = SentryEvent.alloc().initWithLevel(
      this._convertSentryLevel(level)
    );
    event.message = message;

    if (options && options.extra) {
      event.extra = NSDictionary.dictionaryWithDictionary(
        options.extra as NSDictionary<string, any>
      );
    }

    if (options && options.tags) {
      event.tags = NSDictionary.dictionaryWithDictionary(
        options.tags as NSDictionary<string, string>
      );
    }
  ...
  public static captureException(exception: Error, options?: ExceptionOptions) {
    const event = SentryEvent.alloc().initWithLevel(
      SentrySeverity.kSentrySeverityError
    );
  ...
  public static setContextUser(user: SentryUser) {
    ...
    if (user.data) {
      ...
      const dict = NSDictionary.dictionaryWithDictionary(
        user.data as NSDictionary<string, any>
      );
  ...
  public static setContextTags(tags: any) {
    SentryClient.sharedClient.tags = NSDictionary.dictionaryWithDictionary(
      tags as NSDictionary<string, string>
    );
  }
  public static setContextExtra(extra: any) {
    SentryClient.sharedClient.extra = NSDictionary.dictionaryWithDictionary(
      extra as NSDictionary<string, any>
    );
  }
