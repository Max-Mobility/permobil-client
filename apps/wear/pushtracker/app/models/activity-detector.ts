...
export class ActivityDetector {
  ...
  private activityDetectorOutput: java.util.Map<java.lang.Integer, java.lang.Object> = null;
  ...
  constructor() {
    try {
      ...
      this.activityDetectorInput[ActivityDetector.Input_InputIndex] = this.inputData;
      this.activityDetectorInput[ActivityDetector.Input_StateIndex] = this.previousState;
      ...
      this.activityDetectorOutput = new java.util.HashMap<java.lang.Integer, java.lang.Object>();
      ...
      inputShapes[ActivityDetector.Input_StateIndex] = ActivityDetector.StateSize;
      ...
      for (let i = 0; i < inputCount; i++) {
        ...
        if (inputShapes[i] !== inputShape[1]) {
          Log.E(`ActivityDetector::ActivityDetector(): input tensor ${dataType} at ${i}  misconfigured!\n` +
      ...
      outputShapes[ActivityDetector.Output_StateIndex] = ActivityDetector.StateSize;
      ...
      for (let i = 0; i < outputCount; i++) {
        ...
        if (outputShapes[i] !== outputShape[1]) {
          Log.E(`ActivityDetector::ActivityDetector(): output tensor ${dataType} at ${i}  misconfigured!\n` +
  ...
  public detectActivity(acceleration: any, timestamp: number) {
    try {
      ...
      // run the inference
      this.tflite.runForMultipleInputsOutputs(this.activityDetectorInput, this.activityDetectorOutput);
      // get the prediction
  ...
  private getActivity(timestamp: number) {
    ...
    if (this.lastActivityTime !== null) {
      ...
      const timeDiffThreshold = ActivityDetector.ActivityLockoutTimeMs * 1000 * 1000; // convert to ns
  ...
  private updatePredictions(prediction: number) {
    ...
    if (this.predictionHistory.length > ActivityDetector.PredictionHistorySize) {
  ...
  private loadModelFile() {
    ...
    const fileDescriptor = activity.getAssets().openFd(this.activityDetectorModelFileName);
