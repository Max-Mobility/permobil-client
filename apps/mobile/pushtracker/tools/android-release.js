...
askKeystorePassword().then(result => {
  ...
  exec(
    `tns build android --release --env.aot --env.uglify --env.hiddenSourceMap --key-store-path ./pushtracker-keystore.jks --key-store-password ${result} --key-store-alias upload --key-store-alias-password ${result} --aab --copy-to ./pushtracker.aab`,
