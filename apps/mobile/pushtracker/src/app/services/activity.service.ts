    ...
    async loadWeeklyActivity(from: Date, to: Date): Promise<boolean> {
        ...
        try {
            ...
            while (!this._areDaysSame(from, to)) {
                ...
                daysInWeek.push(from.getFullYear() + '/' + (month < 10 ? '0' + month : month) + '/' + (day < 10 ? '0' + day : day));
                from.setDate(from.getDate() + 1);
            }
            const toMonth = to.getMonth() + 1;
            const toDay = to.getDate();
            daysInWeek.push(to.getFullYear() + '/' + (toMonth < 10 ? '0' + toMonth : toMonth) + '/' + (toDay < 10 ? '0' + toDay : toDay));
